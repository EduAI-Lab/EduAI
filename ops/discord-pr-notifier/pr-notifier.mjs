...
const config = {
  ...
  channelId: required("DISCORD_PR_CHANNEL_ID"),
...
async function loadState() {
  ...
  try {
    states = JSON.parse(await readFile(STATE_FILE, "utf8"));
  } catch {
    states = {};
  }
}
...
function parseIssues(body = "") {
  return [
    ...body.matchAll(/(?:close[sd]?|fix(?:e[sd])?|resolve[sd]?)\s+(?:[\w.-]+\/[\w.-]+)?#(\d+)/gi),
...
function prState(pr, repo) {
  ...
  const existing = states[id] ?? {
    ...
    approvers: [],
    completedReviewers: [],
    reviewRequestedAtByReviewer: {},
    reviewerReminderAtByReviewer: [],
    threadId: null,
  };
  states[id] = {
    ...existing,
    ...
    sha: pr.head?.sha,
    linkedIssues: parseIssues(pr.body ?? ""),
    lastActivityAt: now(),
  };
  return states[id];
}
...
async function memberId(login) {
  ...
  const members = await discord(
    `/guilds/${config.guildId}/members/search?limit=10&query=${encodeURIComponent(handle)}`,
    { method: "GET" },
  );
  const match = members.find(
    ({ user }) =>
      user.username.toLowerCase() === handle.toLowerCase() ||
      user.global_name?.toLowerCase() === handle.toLowerCase(),
  );
...
async function send(channelId, text, users = []) {
  ...
  await discord(`/channels/${channelId}/messages`, {
    method: "POST",
    body: JSON.stringify({
      content: `${names.join(" ")}${names.length ? " " : ""}${text}`,
...
async function addToThread(threadId, users = []) {
  ...
  for (const id of ids)
    await discord(`/channels/${threadId}/thread-members/${id}`, { method: "PUT", body: "" });
}
async function removeFromThread(threadId, users = []) {
  const ids = (await Promise.all(users.map(memberId))).filter(Boolean);
  for (const id of ids)
    await discord(`/channels/${threadId}/thread-members/${id}`, { method: "DELETE" });
}
...
async function ensureThread(state) {
  ...
  const issues = state.linkedIssues.length
    ? `Linked issue${state.linkedIssues.length === 1 ? "" : "s"}: ${state.linkedIssues.map((issue) => `#${issue}`).join(", ")}.`
    : "[Action needed] No linked issue found.";
  const starter = await discord(`/channels/${config.channelId}/messages`, {
    method: "POST",
    body: JSON.stringify({
      content: `**PR #${state.number}: ${state.title}**\n${state.url}\n${issues}`,
    }),
  });
  const thread = await discord(`/channels/${config.channelId}/messages/${starter.id}/threads`, {
    method: "POST",
    body: JSON.stringify({
      name: `PR #${state.number} - ${state.title}`.slice(0, 100),
...
async function notify(state, text, users = []) {
  if (state.threadId) await send(state.threadId, text, users);
}
...
async function maybeReady(state) {
  if (
    !state.threadId ||
    !state.ciGreen ||
    state.approvers.length < config.approvals ||
    state.readyAt
  )
    return;
  await notify(
    state,
    "[Ready] Required approvals are present and CI is green. This PR is ready to merge.",
    [state.author],
  );
  state.readyAt = now();
}
async function handle(event, payload) {
  ...
  if (event === "check_suite" && payload.action === "completed") {
    for (const reference of payload.check_suite?.pull_requests ?? []) {
      ...
      if (isFailure(payload.check_suite.conclusion) && state.lastCiFailureSha !== sha) {
        state.lastCiFailureSha = sha;
        state.ciGreen = false;
        await notify(
          state,
          `[CI failing] Commit ${sha?.slice(0, 7) ?? "unknown"} has a failing check.`,
          [state.author],
        );
        ...
        if (state.lastCiPassSha !== sha) {
          ...
          await notify(
            state,
            `[CI passed] All checks passed for commit ${sha?.slice(0, 7) ?? "unknown"}.`,
          );
        }
  ...
  if (event === "pull_request") {
    ...
    if (payload.action === "review_requested" && payload.requested_reviewer?.login) {
      ...
      state.reviewers = unique([...state.reviewers, reviewer]);
      state.reviewRequestedAtByReviewer = {
        ...state.reviewRequestedAtByReviewer,
        [reviewer]: Date.now(),
      };
      state.completedReviewers = (state.completedReviewers ?? []).filter(
        (login) => login !== reviewer,
      );
      state.reviewerReminderAtByReviewer = (state.reviewerReminderAtByReviewer ?? []).filter(
        (entry) => entry.login !== reviewer,
      );
    ...
    if (payload.action === "converted_to_draft") {
      await setThreadArchived(state.threadId, true);
      await notify(state, "[Draft] This PR was moved back to draft; the thread is archived.");
    }
    if (payload.action === "ready_for_review") {
      await setThreadArchived(state.threadId, false);
      await addToThread(state.threadId, state.reviewers);
      await notify(state, "[Ready for review] This PR is no longer a draft.", state.reviewers);
    }
    if (payload.action === "synchronize") {
      state.approvers = [];
      state.sha = pr.head?.sha;
      state.ciGreen = false;
      state.lastAuthorCommitAt = now();
      delete state.readyAt;
    }
    if (pr.mergeable_state === "dirty" && !state.hasConflict) {
      state.hasConflict = true;
      await notify(state, "[Merge conflict] This PR now has merge conflicts.", [state.author]);
    }
  ...
  if (event === "pull_request" && payload.action === "closed") {
    await notify(
      state,
      pr.merged
        ? `[Merged] This PR was merged. Closing the PR thread.`
        : `[Closed] This PR was closed. Removing the PR thread.`,
    );
    try {
      await discord(`/channels/${state.threadId}`, { method: "DELETE" });
    } catch {
      // If the bot lacks Manage Threads, archive and lock as a safe fallback.
      await discord(`/channels/${state.threadId}`, {
        method: "PATCH",
  ...
  if (event === "pull_request_review" && payload.action === "submitted") {
    ...
    const decision = payload.review?.state?.toLowerCase();
    if (reviewer) {
      await addToThread(state.threadId, [reviewer]);
      state.completedReviewers = unique([...(state.completedReviewers ?? []), reviewer]);
      state.reviewerReminderAtByReviewer = (state.reviewerReminderAtByReviewer ?? []).filter(
        (entry) => entry.login !== reviewer,
      );
    }
    if (decision === "changes_requested") {
      state.changesRequestedAt = now();
      delete state.changesReminderAt;
      await notify(
        state,
        `[Changes requested] ${reviewer ?? "A reviewer"} requested changes on this PR.`,
        [state.author],
      );
    }
    if (decision === "approved" && reviewer) {
      state.approvers = unique([...state.approvers, reviewer]);
      await maybeReady(state);
    }
  }
  await saveState();
...
async function reminders() {
  ...
  for (const state of Object.values(states)) {
    ...
    if (
      state.changesRequestedAt &&
      (!state.lastAuthorCommitAt || state.lastAuthorCommitAt < state.changesRequestedAt) &&
      businessDays(state.changesRequestedAt, current) >= 2 &&
      (!state.changesReminderAt || businessDays(state.changesReminderAt, current) >= 2)
    ) {
      await notify(state, "[Reminder] Requested changes are awaiting an author update.", [
        state.author,
      ]);
      state.changesReminderAt = now();
    }
    ...
    for (const reviewer of state.reviewers ?? []) {
      ...
      if (
        assignedAt &&
        Date.now() - assignedAt >= 24 * 60 * 60 * 1000 &&
        (!lastReminder || Date.now() - lastReminder >= 24 * 60 * 60 * 1000)
      ) {
        await notify(state, "[Reminder] Friendly review reminder.", [reviewer]);
        state.reviewerReminderAtByReviewer = [
          ...remindersSent.filter((entry) => entry.login !== reviewer),
          { login: reviewer, at: Date.now() },
        ];
      }
    }
    if (
      state.lastActivityAt &&
      businessDays(state.lastActivityAt, current) >= 5 &&
      (!state.idleReminderAt || businessDays(state.idleReminderAt, current) >= 5)
    ) {
      await notify(
        state,
        "[Idle PR] No review progress in five business days. Please update, request review, or close it.",
        [state.author],
      );
      state.idleReminderAt = now();
    }
  }
  await saveState();
}
...
createServer(async (request, response) => {
  if (request.method !== "POST" || request.url !== "/github/pr-webhook") {
  ...
  try {
    await handle(request.headers["x-github-event"], JSON.parse(body));
    response.writeHead(204).end();
  } catch (error) {
    console.error(error);
    response.writeHead(500).end("Notifier failed");
  }
