...
- Invitation reminders (#724) run as the `notify-invitation-expiry` CORE cron job; lead time is derived from `INVITE_EXPIRY_HOURS`, never hardcoded. There is no `reminderSentAt` column, so the UTC-day window is the only once-only guard: a skipped scheduler run means a skipped reminder, and sub-24h TTLs are only partially covered. The reminder email cannot contain an accept link because `Invitation` stores only `tokenHash`.
