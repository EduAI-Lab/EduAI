...
echo "=== stopping extension dev processes ==="
for s in ext-aitutor ext-aitutor-fe ext-qm-be ext-qm-fe; do
  tmux kill-session -t "$s" 2>/dev/null || true
done
for p in 3001 4000 5173 8000; do
  kill_port "$p"
done
sleep 2

echo "=== starting extension apps ==="
...
# Core: restart only if down
if port_up 3000; then
  echo "up: eduai (:3000)"
else
  tmux kill-session -t eduai 2>/dev/null || true
  kill_port 3000
