...
2. **`X-EduAI-Internal-Key`** — shared secret in `infra/cmps01/.env` and `CMPS01_INTERNAL_KEY` on s378 / research laptops.
...
s378: `ENERGY_SIDECAR_URL=http://cmps01.ok.ubc.ca:8001/energy` and `CMPS01_INTERNAL_KEY=<same as cmps01 .env>` — no separate firewall port for 9100.
