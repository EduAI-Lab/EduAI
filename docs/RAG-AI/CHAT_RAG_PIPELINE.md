...
- **Similarity threshold** defaults from **`RAG_SIMILARITY_THRESHOLD`** (default **0.5**).
...
`buildCappedRagContextText` sorts by similarity (query order), adds `**Source**: {materialTitle}` headers, joins with `---`, and truncates the last chunk with `…` when over budget.
...
2. **Pgvector SQL** — `material_embeddings` → `material_chunks` → `course_materials`, filtered by `courseId`, similarity `1 - (embedding <=> query)`, threshold from **`RAG_SIMILARITY_THRESHOLD`** (default **0.5**), `ORDER BY similarity DESC`, `LIMIT` from caller.
...
| `RAG_SIMILARITY_THRESHOLD` | 0.5 | Minimum cosine similarity for retrieval hits |
...
| Auto routing | Not on this branch; client sends `model` as-is |
