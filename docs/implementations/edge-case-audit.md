...
| AUTH-04 | Last-admin floor: demote/deactivate must leave ≥1 active ADMIN; concurrent demotions serialized. | Boundary (self-ref) | Fail-closed | COVERED | `users-api.server.ts` (`pg_advisory_xact_lock`); `users.rbac.test.ts`; `users-admin-floor.integration.test.ts` | Covered (#1198) | — |
