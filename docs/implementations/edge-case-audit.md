...
| RAG-04 | Concurrent same-checksum ingest: `@@unique([courseId, checksum])` + `P2002` (not duplicate corpus). Since #949 the resolution depends on *which* checksum collides: the provisional `pending:<bytehash>` still yields a synchronous 409 (or an atomic reclaim of a stranded row), while a content-checksum collision is only detectable after the 202 and resolves as a terminal FAILED receipt carrying `duplicateOfId`. | Integration | Fail-closed | COVERED | `schema.prisma`; `courses.materials.test.ts` (RAG-04) | Covered (#1196) | — |
