...
Instructors are linked to their courses via an `Enrollment` row with `role=INSTRUCTOR` — there is no `Course.professorId` field. This is what enables multiple instructors per course.
...
- `Course.professorId` has been removed; instructor links are expressed via `Enrollment` rows with `role=INSTRUCTOR`, enabling multiple instructors per course.
...
| Operation | Target | Current | Notes |
|---|---|---|---|
| Create course | ADMIN, UNIT_ADMIN(D) | **ADMIN only, broken** | `UNIT_ADMIN` route path missing. The old handler wrote `professorId = session.user.id` against a field that no longer exists in the schema — current creation either errors on the missing column or leaves the new course with no instructor enrollment. Replacement contract (`instructorUserIds: string[]`, transactional Enrollment creation) is specced in #298. |
...
| Edit course | ADMIN, UNIT_ADMIN(D), INSTRUCTOR(C) | Stale | Old handler keyed off `professorId` which no longer exists; needs rewrite against `Enrollment.role=INSTRUCTOR`. `UNIT_ADMIN` path also missing. |
| Publish / unpublish | ADMIN, UNIT_ADMIN(D), INSTRUCTOR(C) | Stale | Same `professorId` issue as edit. |
...
| Core course creation is broken after `professorId` removal | Core | Medium — old handler wrote to a removed column; needs replacement with `instructorUserIds` + transactional Enrollment creation per #298 |
