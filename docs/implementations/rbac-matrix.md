...
Authorization is the middleware's job; the service layer scopes course-owned resources by the authorized course's owner id (`req.qmCourse.userId`), which is defense-in-depth, not the gate. Bug-report triage access is now role-based (`ADMIN` only), replacing the `BUG_REPORT_ADMIN_EMAILS` email allowlist.
