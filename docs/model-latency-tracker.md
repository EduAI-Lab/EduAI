...
- Eight ADHD-relevant principles (Mental Demand / Effort hooks): `docs/literature/adhd-design-principles.md`
- Architecture phases — Phase 2.5 owns this work: `docs/literature/adhd-assist-architecture-phases.md`
- Synthetic scenarios used as the canonical probes: `docs/literature/form-a-eval-scenarios.md`, `docs/literature/form-a-scenario-test-sheet.md`
- Manual eval logging conventions (matrix columns): `docs/literature/system-prompt-evaluation-runbook.md`
...
- **`GOOGLE_GENERATIVE_AI_API_KEY` in `.env`** — embeddings, indexing, and **boot warmup** (`[warmup] ...`) also call Google; they often **share one project quota** with the chat key if it is the same key or same billing project.
