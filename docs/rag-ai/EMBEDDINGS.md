...
`RAG_IVFFLAT_PROBES` defaults to 10 and is clamped to `[1, 100]`; zero and invalid values use the code default of 10 (not Postgres' own GUC default of 1). A value of 100 is a valid, fully-index-scan mode that visits every IVFFlat list (`probes == lists` does **not** degrade to a sequential scan). On pgvector 0.8+, retrieval also enables bounded iterative scanning. After the first bulk embedding, rebuild the index with `REINDEX INDEX CONCURRENTLY "material_embeddings_embedding_ivfflat_idx"` so its centroids are trained from real rows.
...
**Filtered-recall mitigation:** a plain fixed-size candidate pool is not sufficient — IVFFlat can exhaust its probed lists before a course filter (`WHERE cm."courseId" = ...`) finds enough matching rows, especially for a small course inside a large shared table. Both paths additionally enable `ivfflat.iterative_scan = relaxed_order` with a bounded `ivfflat.max_probes`, so pgvector keeps expanding the scan to more lists until the post-filter `LIMIT` is satisfied (or the ceiling is hit) instead of returning short. This only applies on pgvector ≥ 0.8.0 (see version gating below); older extensions silently skip both settings and fall back to the fixed-probe behavior.
...
| `ivfflat.probes` | Runtime GUC, `RAG_IVFFLAT_PROBES` env var | `10` | The *initial* number of `lists` clusters a query scans — higher = better recall/closer to exact search, at the cost of scanning more rows. Set per-query via a single chained `SELECT set_config(...)` call inside a `prisma.$transaction` in `findRelevantContent()` (see `resolveIvfflatProbes()` in `embedding.ts`), since it's a session-scoped setting and Prisma pools connections — a bare `$executeRaw` `SET` before a separate `$queryRaw` is not guaranteed to land on the same connection, and collapsing the GUC statements into one call keeps the transaction to 2 round trips instead of 3+. Values `0` and invalid input use the code default of `10`; valid values are clamped to `[1, 100]` (matches `lists = 100`; scanning more lists than exist is meaningless). |
| `ivfflat.max_probes` | Runtime GUC, code constant (not env-overridable) | `32768` | A **different** knob from `ivfflat.probes` above: the *ceiling* iterative scanning is allowed to raise the effective probe count to while chasing enough post-filter rows. Must be `>= lists` (100) or a course filter can never reach the last list; pinned to pgvector's own documented default/max of 32768 for headroom well above any planned `lists` re-tune. Only applied when pgvector `>= 0.8.0` (`ivfflat.iterative_scan` gate below). |

**pgvector version gating:** `ivfflat.iterative_scan` and `ivfflat.max_probes` are pgvector ≥ 0.8.0 GUCs; an older extension rejects them with `unrecognized configuration parameter`, which would abort the whole retrieval transaction and take RAG down silently on every call. Because the repo's Postgres image tag doesn't pin an extension version, `findRelevantContent()` reads `SELECT extversion FROM pg_extension WHERE extname = 'vector'` once per process (cached — not re-checked on every retrieval call) and only enables those two GUCs when the parsed version is `>= 0.8.0`. The version string is parsed into a numeric `[major, minor, patch]` triple and compared in JS rather than in SQL (`string_to_array(...)::int[] >= ARRAY[...]::int[]` is a real trap: a plain element-wise array compare treats a shorter array, e.g. `"0.8"` → `{0,8}`, as less than `{0,8,0}`, and mixing an uncast `text[]` against an `int[]` throws `operator does not exist` outright). On an extension below 0.8.0, retrieval still runs — it just skips iterative scanning and falls back to the fixed `ivfflat.probes` behavior.
...
**Live evidence (#940 issue close-out):** [`IVFFLAT_EXPLAIN_EVIDENCE.md`](./IVFFLAT_EXPLAIN_EVIDENCE.md) captures a real `EXPLAIN (ANALYZE, BUFFERS)` run against a seeded, skewed 50,006-row `pgvector/pgvector:pg16` database (20 large "noise" courses vs. one small target course) for both the pure-vector and hybrid retrieval paths — including the reviewer-reported filtered-recall gap reproduced live (0 rows at `probes=1` without iterative scanning) and its fix (6/6 rows matching exact search with `ivfflat.iterative_scan = relaxed_order` + `ivfflat.max_probes = 32768`).
