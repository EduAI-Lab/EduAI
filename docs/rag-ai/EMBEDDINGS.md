...
`RAG_IVFFLAT_PROBES` defaults to 10 and is clamped to `[1, 100]`; zero and invalid values use the code default of 10. A value of 100 is an explicit maximum-recall mode that scans all IVFFlat lists. On pgvector 0.8+, retrieval also enables bounded iterative scanning. After the first bulk embedding, rebuild the index with `REINDEX INDEX CONCURRENTLY "material_embeddings_embedding_ivfflat_idx"` so its centroids are trained from real rows.
