...
2. **Retrieval SQL** — branches on **`RAG_HYBRID_BM25`**:
   - **Hybrid path** (`RAG_HYBRID_BM25=1`, recommended): pre-filters with the same vector cosine floor as the pure-vector path (`1 − (embedding <=> query) > threshold`), then ranks survivors by combined score: `(1 − (embedding <=> query)) × α + ts_rank(content_tsv, query) × (1−α)`, where α = **`RAG_HYBRID_BM25_ALPHA`** (default **0.7**) and `content_tsv` is the stored, GIN-indexed generated column described below (#941). `ORDER BY score DESC`, `LIMIT`. Improves label queries ("assignment 4") and vague queries ("I'm confused about what prof said") that pure vector search misses, without returning top-k chunks that fail the similarity floor.
...
| BM25 hybrid retrieval | `RAG_HYBRID_BM25=1` combines vector + full-text BM25 ranking after the same vector similarity floor as pure-vector search; `ts_rank`/`plainto_tsquery` read `material_chunks.content_tsv`, a generated (`GENERATED ALWAYS ... STORED`) `tsvector` column with a GIN index (#941) — no per-query `to_tsvector(content)` tokenization. Recommended for production. |
