...
| `SESSION_VALIDATE_PREAUTH_RATE_LIMIT` | optional (default 1200) | dev/prod | Coarse per-proxy-client-IP admission ceiling for `POST /api/sessions/validate`, enforced before Better Auth session lookup. Keep this above the post-auth per-user limit so normal extension traffic is not collapsed into one low shared bucket. |
