...
| `COMPLETION_FLEET_FALLBACK_MODELS` | optional (default `openai:gpt-4o-mini,google:gemini-2.5-flash`) | dev/prod | Ordered `provider:model` list `/api/completion` falls back to when the UBC vLLM fleet is unavailable (#1645). Only a candidate whose BYOK key the caller supplied is used; local providers (`vllm`/`ollama`) are ignored. With no usable key the outage stays `MODEL_UNAVAILABLE`. |
