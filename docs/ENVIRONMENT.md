...
| `API_KEY_RATE_LIMIT_MAX`, `API_KEY_RATE_LIMIT_WINDOW_MS`, `API_KEY_RATE_LIMIT_ENABLED` | optional (defaults `1000` / `86400000` / on) | dev/prod | Per-key daily ceiling applied by the Better Auth api-key plugin to every `x-api-key` request, independent of the `CHAT_RATE_LIMIT` burst window above. **The plugin rate-limits keys whether or not Core configures it, and its own defaults are 10 requests per 24 hours** — which is what every EduAI key silently received before #1803. The plugin stamps `MAX`/`WINDOW_MS` onto each key at creation time; changing these env vars does not affect keys that already exist. Denials answer `429 {"error":"RATE_LIMITED","retryAfter":<seconds>}` with `Retry-After`; a key whose `remaining` counter is spent answers `429 {"error":"USAGE_EXCEEDED"}` with no retry hint. Set `API_KEY_RATE_LIMIT_ENABLED=0` (exactly `0`) only for load testing against a disposable deployment. A non-positive or unparseable `MAX`/`WINDOW_MS` falls back to the default rather than locking every key out. |
