...
The suite fails closed unless the base URL is exactly the allowlisted HTTPS origin and the course id is exactly `204888`. It requires an existing instructor user identified by `CANVAS_LIVE_CORE_USER_ID`; that identity is the only Core identity used by the test. Canvas operations are GET-only. The Core sync is restricted to the designated course and uses existing upsert/staging behavior; it does not delete or unsync shared dev data.
