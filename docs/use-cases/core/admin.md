...
- **Expected outcome:** `200` (SSR page render) showing only the requested tab/filter's rows, redaction already applied at write time (`sanitizeSensitiveData` in `apps/core/app/lib/redact.server.ts` strips credential/token/secret-shaped `details` keys *and* scrubs secret-shaped values out of every string leaf; `createSystemLog` applies the same pass to `message` and `stack` at the write chokepoint, so none of it is ever persisted).
