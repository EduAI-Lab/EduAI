...
- **Flow:**
  ...
  3. `uploadMaterial` calls `validateUploadedFile` (`apps/core/app/lib/ai/file-processing.ts`) inline — MIME type/size caps (`validateFile`) plus the declared-MIME-vs-magic-bytes sniff. This is the only part of the pipeline that still runs on the request; a rejected file fails fast with `400` instead of becoming a background `FAILED` row (#949)
  4. A `CourseMaterial` row is created with `status: "PROCESSING"`, `uploadedBy: user.id`, and a provisional `pending:<sha256-of-bytes>` checksum (the *content* checksum is not known until extraction runs). The raw bytes are written alongside it as a `MaterialUploadBlob` in the same statement, so the upload survives a restart. The upload is audit-logged (`MATERIAL_UPLOADED`) immediately, independent of extraction or embedding success
  5. The route returns `202` and hands the rest to `runMaterialExtraction` (`lib/materials/extraction-job.server.ts`), which first takes the row's extraction lease: `extractUploadedFileContent` runs the PDF/DOCX/PPTX/TXT/MD extractors — all of them in the isolated, memory- and time-capped subprocess — and `sanitizeTextContent`, then the row is promoted with the real title/MIME/size and the content checksum
  6. `processMaterialEmbeddings(materialId, fileInfo.content)` (`apps/core/app/lib/ai/embedding.ts`) chunks the sanitized text (splitting on `SEMANTIC_CHUNK_SEPARATOR` where present) and batch-embeds each chunk into `material_embeddings` (pgvector)
  7. On success, the material's `status` flips to `"READY"` with `processedAt` set. The client (`useCourseMaterials.watchUpload`) polls the materials list until the row leaves `PROCESSING`
- **Expected outcome:** `202 { success: true, materialId, status: "PROCESSING", message: "Material accepted and is being processed" }`, followed by the row reaching `READY` under polling. The material is then retrievable via `findRelevantContent` for RAG chat on course 42.
- **Failure modes / what could go wrong:** Everything after the `202` is reported through the row, not the HTTP status. If extraction throws, the row goes `FAILED` with a `MATERIAL_EXTRACT_FAILED` system error; if `processMaterialEmbeddings` throws, `FAILED` with `MATERIAL_EMBED_FAILED` — in both cases the row still exists (visible to staff, not embedded), it does not silently vanish. If extraction finds the content already on the course, the provisional row becomes a terminal `FAILED` receipt with `duplicateOfId` pointing at the winner, which is the asynchronous successor to the old synchronous `409`; the client reads that outcome and deletes the receipt. A restart mid-extraction is recoverable rather than terminal: the bytes are persisted and the row's `extractionLeaseUntil` expires, after which the sweeper re-claims it and re-runs extraction from those bytes. After `MAX_EXTRACTION_ATTEMPTS` the row is failed as `MATERIAL_EXTRACT_ABANDONED` instead of being retried forever. Re-uploading the same bytes also reclaims the row directly (see `reclaimProvisionalRow`); the two recovery paths are independent.
- **Related code:**
