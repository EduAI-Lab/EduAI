...
- **Flow:**
  ...
  4. A `CourseMaterial` row is created with `status: "PROCESSING"`, `uploadedBy: user.id`, and a provisional `pending:<sha256-of-bytes>` checksum (the *content* checksum is not known until extraction runs). The upload is audit-logged (`MATERIAL_UPLOADED`) immediately, independent of extraction or embedding success
  5. The route returns `202` and hands the rest to `processMaterialAsync`, started fire-and-forget: `extractUploadedFileContent` runs the PDF/DOCX/PPTX/TXT/MD extractors and `sanitizeTextContent`, then the row is promoted with the real title/MIME/size and the content checksum
...
- **Failure modes / what could go wrong:** Everything after the `202` is reported through the row, not the HTTP status. If extraction throws, the row goes `FAILED` with a `MATERIAL_EXTRACT_FAILED` system error; if `processMaterialEmbeddings` throws, `FAILED` with `MATERIAL_EMBED_FAILED` — in both cases the row still exists (visible to staff, not embedded), it does not silently vanish. If extraction finds the content already on the course, the provisional row becomes a terminal `FAILED` receipt with `duplicateOfId` pointing at the winner, which is the asynchronous successor to the old synchronous `409`; the client reads that outcome and deletes the receipt. Because the background task is in-process fire-and-forget, a restart mid-extraction strands the row in `PROCESSING`; re-uploading the same bytes reclaims it (see `reclaimProvisionalRow`), and no sweeper exists yet (tracked on #949).
- **Related code:**
