...
- **Expected outcome:** `200`/streaming response for a general, non-course-scoped chat, attributed to the synthetic `"service"` user id rather than any real person, with no `Chat`/message row ever persisted for the request.
- **Failure modes / what could go wrong:** This is verified, not open: the `chats_userId_fkey` constraint does exist (`Chat.userId` is a real FK to `User.id`), and the literal string `"service"` is not a `User` row, so persisting a `Chat`/message here would raise Prisma error `P2003`. Core avoids this deliberately rather than by accident — `apps/core/app/routes/api/chat.ts` lines 586-590 set `const ephemeral = isServiceKeyCaller;` with an explicit comment: *"Service-key (server-to-server) callers ... are stateless and have no real User row, so persisting a Chat would violate chats_userId_fkey (P2003). Skip all chat/message persistence for them and run the model against the incoming messages only."* Every persistence branch later in the handler is gated on `!ephemeral`, so no `Chat` or message row is ever written for this caller; the response is generated and streamed back without a database write. The remaining open question is narrower than "does this violate an FK": whether any `AIInteraction` logging path is *also* gated on `ephemeral`/`isServiceKeyCaller` the same way, or whether `AIInteraction` rows can still be created for the synthetic `"service"` user — that was not re-traced here and would need its own check of the `AIInteraction`-writing code path.
- **Related code:**
  - `apps/core/app/routes/api/chat.ts`
