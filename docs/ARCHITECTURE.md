...
flowchart TD
  ...
  Parse --> SDK
  Reg --> SDK
  SDK --> Vendor[Google / OpenAI / OpenRouter / Ollama / vLLM APIs]
```
...
| `apiKeys.openrouter.apiKey` in `/api/chat` body             | **Chat** via OpenRouter gateway (models like `openrouter:openai/gpt-4o`) | Client/request or chat API key settings (browser localStorage)                      |
| `OPENROUTER_API_KEY`                                      | **Chat** via OpenRouter when client has no browser key           | Server `.env` only (merged in `/api/chat`)                                           |
