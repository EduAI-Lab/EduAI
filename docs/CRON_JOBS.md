...
|---|---|---|---|
| **Infra shell scripts** | Spawned by a Core process (the dedicated cron worker, or a web server) | Core's `node-cron` scheduler + a Postgres execution lease | Yes |
| **Core handlers** | In-process inside Core (no shell script) | Same scheduler; `execution: "CORE"` in the registry | Yes |
...
The panel uses the shared cron-status poller: it refreshes every 15 seconds while jobs are active and backs off to 30 seconds when idle. The sidebar and admin page share this same source, so they cannot start competing polling loops.
...
Schedule changes are stored in the database and picked up by the dedicated cron worker during its next 30-second reconciliation cycle. No web-server restart is needed; the worker owns scheduling independently of web requests.
...
Saving the schedule updates the database. The worker applies the change during its next 30-second reconciliation cycle, so no deployment or server restart is needed. A yellow "custom" badge appears on the row.
...
This direct invocation creates its own audit row and execution lease. The lease owner, heartbeat, expiry, and timestamps are generated by PostgreSQL; the default lifetime is 10 minutes (`CRON_STANDALONE_LEASE_MS` in `/etc/eduai/cron.env`). If the start row cannot be inserted, the script exits before doing work. `cron_finish` and `cron_fail` only terminalize a `RUNNING` row while the same owner still holds an unexpired lease, and clear all lease columns when they succeed. Set the lease longer than the slowest direct run.
Scripts launched by Core receive `CORE_CRON_RUN_ID` and skip this path because Core owns their lease and final status.

## Execution safety and crash recovery

Core starts the scheduler when the server process loads. The scheduler does not wait for an HTML page or API request. Each Core replica can schedule the same expression, but PostgreSQL grants one live execution lease per job. Losing replicas reuse the winner's run ID and do not start another child process.

The child renews its lease while it runs. If a process crashes, the lease expires. Server startup or the next trigger changes the expired attempt to `ERROR` and creates one successor.
The expired row remains in history. An old process cannot renew or finish a run after its owner token or lease expires.

Core retains at most `CRON_OUTPUT_MAX_BYTES` of combined stdout and stderr (64 KiB by default).
If a script exceeds the cap, Core sends `SIGTERM`, escalates to `SIGKILL` after five seconds if needed, and records a redacted output-limit error. Set `CRON_RUN_LEASE_MS` and `CRON_OUTPUT_MAX_BYTES` in `apps/core/.env`; see `docs/ENVIRONMENT.md` for valid ranges.
...
```ts
{
  ...
  script: "my-new-job.sh",     // "" for an extension job; execution: "CORE" for an in-process handler
},
```

The full `KnownCronJob` shape is `{ name, description, schedule, scheduleLabel, script, execution?: "SCRIPT" | "CORE", triggerEnabled?: boolean }`. `execution` defaults to `SCRIPT`; `triggerEnabled` defaults to true.

The cron worker discovers it during the next 30-second reconciliation cycle. The job will appear in the admin panel, can be triggered manually (and dispatched under `eduai-cron`), and has its schedule editable via the UI.
