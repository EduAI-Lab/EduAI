...
This direct invocation creates its own audit row and execution lease. The
lease owner, heartbeat, expiry, and timestamps are generated by PostgreSQL;
the default lifetime is 10 minutes (`CRON_STANDALONE_LEASE_MS` in
`/etc/eduai/cron.env`). If the start row cannot be inserted, the script exits
before doing work. `cron_finish` and `cron_fail` only terminalize a `RUNNING`
row while the same owner still holds an unexpired lease, and clear all lease
columns when they succeed. Set the lease longer than the slowest direct run.
Scripts launched by Core receive `CORE_CRON_RUN_ID` and skip this path because
Core owns their lease and final status.
