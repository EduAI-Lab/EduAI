...
- Added Core support for a local OpenAI-compatible embedding endpoint through `VLLM_EMBEDDING_BASE_URL` and `VLLM_EMBEDDING_MODEL`. `CMPS01_INTERNAL_BASE_URL` independently allowlists the CMPS edge before `CMPS01_INTERNAL_KEY` can be attached; the candidate embedding URL is never its own trust source.
...
- Queue processing is intentionally disabled for the initial Core release. When a worker is deployed and verified, update the production queue variables—especially `QUEUE_ENQUEUE_ENABLED=true`—as part of the worker enablement change, then restart the Core service.
  - **Correction (2026-08-31, verified in code):** `QUEUE_ENQUEUE_ENABLED` no longer does anything. The queue is disabled *in code*, not by configuration: `isEnqueueRequested()` (`app/lib/queue/chat-producer.server.ts`) ignores the variable outright, and `npm run queue:worker` calls `assertAiJobQueueEnabled()` and exits before constructing Prisma, Redis, or any BullMQ worker. Enabling the queue is a reviewed code change, not an env edit. Redis itself is still required in production — it backs the live `/api/chat` + `/api/completion` rate limits.
