...
**IURA / ADHD Assist build:** phased plan, Form A alignment, **streaming-first** oversight, and **Phase 2.5** (§3b context / tool efficiency) live in [`literature/adhd-assist-architecture-phases.md`](./literature/adhd-assist-architecture-phases.md). Before you implement: **sync with GitHub** (`git fetch` / pull or rebase onto `main`), then note the commit SHA for your appendix.
