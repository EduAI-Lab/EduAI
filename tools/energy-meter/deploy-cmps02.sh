#!/usr/bin/env bash
# Deploy energy sidecar on cmps02 via Docker (same pattern as deploy-cmps01.sh).
set -euo pipefail
...
curl -sf -X POST "http://127.0.0.1:${ENERGY_METER_PORT:-9100}/measure-stop" \
  -H 'Content-Type: application/json' -d "{\"tag\":\"$TAG\"}"
echo ""
echo "=== done — then: cd ~/cmps02 && ./deploy-edge-proxy.sh ==="
echo "=== s378: ENERGY_SIDECAR_URL=http://cmps02.ok.ubc.ca:8001/energy ==="
