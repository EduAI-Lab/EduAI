...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v1.2): greeting/meta allowlists, course-intent coding help, strong metadata overlap (no bare-topic false positives), weak-RAG bypass rules, foundational STEM allowance, default hard-refuse for off-topic zero-hit turns, `buildCourseScopePromptBlock`, and `buildScopeRefusalMessage`. |
| `chat-always-on-rag.route.test.ts` | Route tests for smart course RAG on `POST /api/chat` (#484): prefetch on course-scoped turns; inject grounding block for course-intent or strong-similarity hits; skip inject for greetings/generic queries with weak hits; hard-refuse off-topic with zero/weak hits (#729 v1.2); tool path uses same gate and caps `maxTokens` against model DB limit. |
| [`chat-scope.route.test.ts`](apps/core/app/tests/unit/chat-scope.route.test.ts) | Route tests for course scope gate (#729 v1.2) on `POST /api/chat`: off-topic + zero/weak RAG hits returns canned refusal without calling the model; greetings, foundational, and course-context coding requests proceed; on-topic hits inject scope policy into system prompt. |
