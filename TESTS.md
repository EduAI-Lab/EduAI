...
| `adhd-oversight.test.ts` | Tests `isAdhdOversightEnabled` env parsing, `tryDeterministicStructuralFix` on the archived S1-on baseline drift fixture, `auditAndMaybeRewrite` pass-through for compliant drafts, deterministic rewrite without LLM, and LLM fallback when deterministic fix cannot restore structural pass. |
