...
| [`ChatMessage.test.tsx`](apps/core/app/tests/unit/ChatMessage.test.tsx) | Verifies user vs AI message rendering — text content, right-aligned user layout, `U`/`AI` avatar fallbacks, AI copy button, Streamdown markdown wiring (`isAnimating` during streaming), and that both mark their content as a reading surface. |
| [`MarkdownRenderer.test.tsx`](apps/core/app/tests/unit/MarkdownRenderer.test.tsx) | Verifies the chat markdown wrapper renders the prose/reading-surface shell and resolves lazy Streamdown content for AI message bodies. |
