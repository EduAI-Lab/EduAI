...
| `invitation-schemas.test.ts` | `createInvitationSchema` — INSTRUCTOR/ADMIN/UNIT_ADMIN/STUDENT accepted, TA rejected as non-invitable, units required for UNIT_ADMIN and rejected for STUDENT and other non-UNIT_ADMIN roles, invalid email / unknown unit code rejection — and `acceptInvitationSchema` min-8 password, confirmPassword match, and required token/name. |
