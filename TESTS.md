...
| [`instructor.publish-confirm.test.tsx`](apps/extensions/ai-tutor/app/tests/unit/instructor.publish-confirm.test.tsx) | Instructor course list: clicking the publish status button opens the confirmation dialog without calling the API; confirming calls `publishCourse`/`unpublishCourse` with the correct id; cancelling does not call the API. |
...
| [`instructor.topic.publish-confirm.test.tsx`](apps/extensions/ai-tutor/app/tests/unit/instructor.topic.publish-confirm.test.tsx) | Instructor topic lesson list: publish button opens the dialog without calling the API; confirming calls `publishLesson`/`unpublishLesson` with the correct id; cancelling does not call the API. |
| [`instructor.list.delete-confirm.test.tsx`](apps/extensions/ai-tutor/app/tests/unit/instructor.list.delete-confirm.test.tsx) | Instructor lesson editor: Remove button opens the delete confirmation dialog without calling the API; confirming calls `deleteActivity` with the correct id; cancelling does not call the API; `toast.error` fires when `deleteActivity` fails. |
| [`CourseEnrollmentsPanel.remove-confirm.test.tsx`](apps/extensions/ai-tutor/app/tests/unit/CourseEnrollmentsPanel.remove-confirm.test.tsx) | Course enrollments panel: Remove button opens the confirmation dialog without calling the API; confirming calls `removeStudentFromCourse` with the correct ids; cancelling does not call the API; the Remove button is disabled while removal is in progress. |
...
| `topics.test.js` | Topics can be listed and created for authorized members, duplicate names and empty values are rejected, students cannot create topics, and activities can be remapped from one topic to another; ADMIN sees topics for any course even when neither enrolled nor assigned (#781) |
