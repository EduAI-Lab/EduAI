...
| `courses.enrollments.test.ts` | `GET /api/courses/:id/enrollments` loader: 400 missing id, 401 no session, 403 invalid service key, 403 user not enrolled, 404 course not found (both auth paths), 200 via service key and user OAuth (STUDENT/INSTRUCTOR), role mapping incl. `id` and decrypted `studentNumber`, null `enrolledAt`, active + inactive returned together, and empty enrollment list. |
