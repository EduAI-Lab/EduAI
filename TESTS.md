...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v2/v2.1/v2.3): deny-list hard-refuse; conversation continuity for ellipsis follow-ups (ascii/binary); career/LinkedIn pivot refusal (crescendo drift); direct payload vs analogy framing; topic-laundering regression; metadata overlap; screenshot regression; `buildCourseScopePromptBlock` and `buildScopeRefusalMessage`. |
