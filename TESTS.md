...
| `chat-always-on-rag.route.test.ts` | Route tests for always-on course RAG policy on `POST /api/chat` (#484): hybrid path calls `findRelevantContent` for any query (including generic "what is…" and greetings) when a course is selected; tool path preloads `findRelevantContent` when `effectiveCourseId` is set; neither path calls `findRelevantContent` when no course is selected. |
