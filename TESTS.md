...
| `canvas-sync-delta.test.ts` | `computeCanvasSyncDelta` check/uncheck logic: newly checked courses, omitted courses unsynced, empty selection unsyncs all, and a synced course that has dropped out of the instructor's live Canvas course list is left untouched rather than unsynced. |
...
| `canvas-fetch-dialog.test.tsx` | Fetch-from-Canvas dialog: shows all courses (synced + unsynced), already-fetched courses render as links to their EduAI course page, unsynced courses render as checkboxes (not links), `syncCanvasCourses` is called with already-synced ids plus newly checked ids, the Fetch selected button is disabled when nothing is checked, per-course sync errors are surfaced instead of failing silently, and the empty state when no Canvas courses exist. |
| `canvas-dashboard-card.test.tsx` | Dashboard Canvas card: disconnected state shows a settings link (no fetch button); connected state shows the "Fetch from Canvas" button and opens the fetch dialog. |
