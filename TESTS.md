...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v3): classifier LLM prompt/parse, async `evaluateCourseScope`, allowlist skips, refusal on `inScope: false`, crescendo regression via test override; `buildCourseScopePromptBlock` and `buildScopeRefusalMessage`. |
| [`course-scope-classifier.ts`](apps/core/app/lib/ai/course-scope-classifier.ts) | Layer B classifier — builds per-course prompt from corpus excerpts + thread, calls `generateText`, parses `{ inScope, reason }` JSON before main chat generation. |
