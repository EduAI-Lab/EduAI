...
| `chat.web-tools.route.test.ts` | `chat.webToolsEnabled` master + `students.canUseWebTool` grant (#660) asserted via the `X-Web-Tools-Enabled` response header: web tools absent when the master is off, present for a non-student when the master is on, and present for a student only when both the master and the student grant are on. |
