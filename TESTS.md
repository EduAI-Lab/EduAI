...
| `invitation-schemas.test.ts` | `createInvitationSchema` — INSTRUCTOR/ADMIN/UNIT_ADMIN/STUDENT accepted, TA rejected as non-invitable (created via enrollment, not a platform role), units required for UNIT_ADMIN and rejected for other roles, invalid email / unknown unit code rejection — and `acceptInvitationSchema` min-8 password, confirmPassword match, and required token/name. |
| `invitation-scoping.test.ts` | Ownership scoping for `listInvitations`/`revokeInvitation`/`resendInvitation` — ADMIN is unscoped (sees and acts on all); a UNIT_ADMIN's list is filtered to `invitedById` and revoke/resend on an invite they didn't create returns `403 FORBIDDEN`. |
...
| `courses.materials.test.ts` | `GET` (student publish gate, 404/403 split via `resolveCourseAccess`), `POST` (STUDENT uploads blocked — TA-and-up only; re-upload of a soft-deleted checksum restores rather than 409), and `DELETE /api/courses/:id/materials/:materialId` (ADMIN/UNIT_ADMIN/INSTRUCTOR plus TA own-only via `uploadedBy`, ownerless rows denied to TAs, **one-way soft delete** setting `deletedAt`/`deletedBy`): the full §7 role × op matrix. |
