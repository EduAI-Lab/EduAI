...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v2): deny-list hard-refuse for clearly off-topic prompts, conversation continuity for follow-ups (ascii/binary), fail-open learning intent, scope RAG affinity, metadata overlap, `buildCourseScopePromptBlock`, and `buildScopeRefusalMessage`. |
...
| [`chat-scope.route.test.ts`](apps/core/app/tests/unit/chat-scope.route.test.ts) | Route tests for course scope gate (#729 v2) on `POST /api/chat`: clearly off-topic returns canned refusal without calling the model; greetings, foundational follow-ups, and course-context coding requests proceed; on-topic hits inject scope policy into system prompt. |
