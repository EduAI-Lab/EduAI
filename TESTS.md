...
| [`ChatInput.test.tsx`](apps/core/app/tests/unit/ChatInput.test.tsx) | Tests the chat composer: assistive mode switch label/toggle, settings gear opens API key dialog, send/stop button enablement and handlers, and textarea placeholder. |
| [`ChatHeaderControls.test.tsx`](apps/core/app/tests/unit/ChatHeaderControls.test.tsx) | Verifies the header no longer hosts the assistive switch (moved to the input) and still exposes the System Prompt control. |
