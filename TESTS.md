...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v1.1): allowlist (greetings, meta, coding), clearly-off-topic hard refuse, course-metadata overlap, soft LLM scope for related zero-hit questions, `buildCourseScopePromptBlock`, and `buildScopeRefusalMessage`. |
| `chat-always-on-rag.route.test.ts` | Route tests for smart course RAG on `POST /api/chat` (#484): prefetch on course-scoped turns; inject grounding block for course-intent or strong-similarity hits; skip inject for greetings/generic queries with weak hits; hard-refuse course-intent with zero hits (#729); tool path uses same gate and caps `maxTokens` against model DB limit. |
| [`chat-scope.route.test.ts`](apps/core/app/tests/unit/chat-scope.route.test.ts) | Route tests for course scope gate (#729) on `POST /api/chat`: off-topic + zero RAG hits returns canned refusal without calling the model; greetings and coding requests proceed; on-topic hits inject scope policy into system prompt. |
