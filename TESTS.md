...
| `departments.test.ts` | `DepartmentSchema` accepts the canonical seeded subject codes and rejects unknown codes, wrong casing, and empty values. |
...
| `users.rbac.test.ts` | `PATCH /api/users/:id` guards: self-role-change 403 (alongside the existing self-deactivation lockout), ADMIN-only `authorizedUnits` assignment validated with `DepartmentSchema`, `422 ROLE_MISMATCH` for non-UNIT_ADMIN targets, and same-request promotion to UNIT_ADMIN. |
