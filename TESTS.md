...
| [`embedding.rag-settings.test.ts`](apps/core/app/tests/unit/embedding.rag-settings.test.ts) | `findRelevantContent` course-level RAG settings: course `ragTopK` overrides the caller's limit; falls back to caller limit then global default when null; course `ragSimilarityThreshold` overrides caller arg and env default; correct `getCourseRagSettings` call shape; output mapped to `{ content, similarity, materialTitle }`. |
| [`courses.rag-settings-cache.test.ts`](apps/core/app/tests/unit/courses.rag-settings-cache.test.ts) | In-memory cache for `getCourseRagSettings`: repeated reads for the same courseId only hit the DB once; expired TTL triggers a fresh DB read; `invalidateCourseRagSettingsCache` forces re-fetch; invalidating one course does not evict others; null (missing course) results are cached; independent cache entries per courseId. |
