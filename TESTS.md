...
| [`course-scope.test.ts`](apps/core/app/tests/unit/course-scope.test.ts) | Unit tests for course scope gate (#729 v2/v2.1): deny-list hard-refuse for clearly off-topic prompts; conversation continuity for follow-ups (ascii/binary); fail-open learning intent and scope RAG affinity; direct payload vs analogy framing (`isDirectOffTopicRequest`, `isAnalogyOrConceptQuestion`); topic-laundering regression (recipe/how-to refused mid-thread even after analogy turn); metadata overlap; screenshot regression; `buildCourseScopePromptBlock` and `buildScopeRefusalMessage`. |
| `chat-always-on-rag.route.test.ts` | Route tests for smart course RAG on `POST /api/chat` (#484): prefetch on course-scoped turns; inject grounding block for course-intent or strong-similarity hits; skip inject for greetings/generic queries with weak hits; hard-refuse clearly off-topic deny-list prompts (#729 v2); tool path uses same gate and caps `maxTokens` against model DB limit. |
| [`chat-scope.route.test.ts`](apps/core/app/tests/unit/chat-scope.route.test.ts) | Route tests for course scope gate (#729 v2/v2.1) on `POST /api/chat`: clearly off-topic returns canned refusal without calling the model; greetings, foundational follow-ups, and course-context coding requests proceed; concept follow-ups after substantive assistant turns; recipe payload refused after digital-literacy thread (#729 v2.1); on-topic hits inject scope policy into system prompt. |
