...
| `adhd-oversight.test.ts` | Tests `isAdhdOversightEnabled` env parsing, redirect preservation on baseline S2 turn 2, forward-offer vs comprehension-check extraction, last inline `Next?` matching, deterministic fixes for archived S1/S2/S3 transcripts, LLM fallback + failure handling, ineligible-draft preservation (non-prose drafts pass through unchanged), over-cap LLM rejection, and `isOversightEligibleDraft` guards. |
