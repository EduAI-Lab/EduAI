...
Async AI jobs expose `GET /api/ai-jobs/:jobId` for owner-scoped status polling. The response includes a live `queuePosition` and an advisory `etaSeconds` derived from recent completions in the job's persisted pool; the estimate is null until that pool has usable timing history.
