...
The tool logs into Core once and reuses that session for all three apps — Better Auth's dev cookie is host-only for `localhost` with no port restriction (RFC 6265), and AI Tutor / Question Maker authenticate every request by forwarding the `Cookie` header to Core's `/api/sessions/validate` rather than keeping their own session. Each result carries an `authOk` flag confirming the navigation actually landed on the target page rather than being bounced to a login screen; the run exits non-zero if any page fails that check. Full rationale in the `isAuthenticatedNavigation()` doc comment in [`scripts/mobile-audit/lib.mjs`](scripts/mobile-audit/lib.mjs).
