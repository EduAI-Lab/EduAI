...
One account can hold more than one **view**. An ADMIN or UNIT_ADMIN who holds a real active `INSTRUCTOR` enrollment on a published course reaches the instructor surface for that course, labelled with a banner and a way back to the admin surface. The switch is navigation, not stored state — nothing is written to `UserPreference` or the session — matching AI Tutor's instructor-to-student preview. It changes presentation only: `resolveCourseAccess` still resolves those accounts to `admin`/`unit`, and `canUseInstructorChatMode` (`app/lib/rbac/instructor-view.server.ts`) admits them **only** on a course they genuinely teach, so an admin who teaches nothing sees no switch and is refused by the API exactly as before.
