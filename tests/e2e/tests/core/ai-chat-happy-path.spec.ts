...
test.describe("Core student AI chat happy path (#1429, #1459)", () => {
  test("enrolled student can ask a course question and read the assistant answer", async ({
    page,
    playwright,
  }) => {
    ...
    try {
      ...
      await page.goto(
        `${CORE_URL}/chat?courseCode=${encodeURIComponent(courseCode)}`,
      );
      ...
      await expect(
        page.getByText("What is the powerhouse of the cell?", { exact: true }),
      ).toBeVisible();
      await expect(
        page.getByText("The mitochondria is the powerhouse of the cell.", {
          exact: true,
        }),
