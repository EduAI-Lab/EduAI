...
test.beforeAll(async ({ playwright }) => {
  ...
  try {
    ...
    expect(
      res.ok(),
      `demo-data seed failed: ${res.status()} ${await res.text()}`,
    ).toBeTruthy();
  } finally {
    await ctx.dispose();
  }
...
test.describe("Student (student1) — UI walkthrough", () => {
  test("dashboard shows only enrolled courses, no TA framing", async ({
    page,
    playwright,
  }) => {
    ...
    try {
      ...
      await page.screenshot({
        path: "test-results/week15/student1-dashboard.png",
        fullPage: true,
      });
  ...
  test("course chat: on-topic + off-topic (course-scope guardrail)", async ({
    ...
    try {
      await injectSession(page, ctx);
      await page.goto(
        `${CORE_URL}/chat?courseCode=${encodeURIComponent("COSC 101")}`,
      );
  ...
  test("enrolled-but-unpublished course (student2/cosc211): list + direct nav", async ({
    ...
    try {
      const listRes = await ctx.get(
        `${CORE_URL}/api/courses?ids=${COURSES.cosc211}`,
      );
      const listBody = await listRes.json();
      const inList = (listBody.data ?? []).some(
        (c: any) => c.id === COURSES.cosc211,
      );
  ...
  test("IDOR probes: cross-course chat, enrollments, questions, chats-by-id", async ({
    ...
    try {
      ...
      const enrOwnRes = await ctx.get(
        `${CORE_URL}/api/courses/${COURSES.cosc101}/enrollments`,
      );
      ...
      const qRes = await ctx.get(
        `${CORE_URL}/api/questions?courseId=${COURSES.cosc101}`,
      );
  ...
  test("materials visibility for an enrolled course", async ({
    playwright,
  }) => {
    const ctx = await newAuthedContext(playwright, USERS.student1);
    try {
      const res = await ctx.get(
        `${CORE_URL}/api/courses/${COURSES.cosc101}/materials`,
      );
  ...
  test("bug reports: own vs guessing another user's report id", async ({
    playwright,
  }) => {
    ...
    try {
      const adminListRes = await adminCtx.get(
        `${CORE_URL}/api/admin/bug-reports`,
      );
      ...
      if (someoneElsesReport) {
        const guessRes = await studentCtx.get(
          `${CORE_URL}/api/bug-reports/${someoneElsesReport.id}`,
        );
        ...
        test
          ...
          .annotations.push({
            type: "finding",
            description: "No other-user bug report found to probe.",
          });
      }
    } finally {
...
test.describe("TA (ta.cs) — UI walkthrough", () => {
  test("dashboard reflects TA framing, not generic Student", async ({
    page,
    playwright,
  }) => {
    ...
    try {
      ...
      await page.waitForLoadState("networkidle");
      await page.screenshot({
        path: "test-results/week15/ta-cs-dashboard.png",
        fullPage: true,
      });
    } finally {
      await ctx.dispose();
  ...
  test("TA course page (cosc101) vs non-TA course (math200)", async ({
    page,
    playwright,
  }) => {
    const ctx = await newAuthedContext(playwright, USERS.taCS);
    try {
      ...
      await page.waitForLoadState("networkidle");
      await page.screenshot({
        path: "test-results/week15/ta-cs-cosc101.png",
        fullPage: true,
      });
      ...
      await page.waitForLoadState("networkidle");
      await page.screenshot({
        path: "test-results/week15/ta-cs-math200-not-ta.png",
        fullPage: true,
      });
      const bodyText = await page.locator("body").innerText();
      test.info().annotations.push({
  ...
  test("TA chat in own course + course-scope guardrail", async ({
    page,
    playwright,
  }) => {
    const ctx = await newAuthedContext(playwright, USERS.taCS);
    try {
      ...
      await page.waitForTimeout(15_000);
      await page.screenshot({
        path: "test-results/week15/ta-cs-chat.png",
        fullPage: true,
      });
    } finally {
      await ctx.dispose();
  ...
  test("TA permission probes: questions (view w/ answers), create question, enroll, admin routes", async ({
    ...
    try {
      ...
      const qOtherRes = await ctx.get(
        `${CORE_URL}/api/questions?courseId=${COURSES.math200}`,
      );
      ...
      const enrollRes = await ctx.post(
        `${CORE_URL}/api/courses/${COURSES.cosc101}/enrollments`,
        {
  ...
  test("TA chat oversight: viewing course chats for own course vs unrelated course", async ({
    ...
    try {
      ...
      const ownCourseChatsRes = await ctx.get(
        `${CORE_URL}/api/courses/${COURSES.cosc101}/chats`,
      );
      ...
      const otherCourseChatsRes = await ctx.get(
        `${CORE_URL}/api/courses/${COURSES.math200}/chats`,
      );
