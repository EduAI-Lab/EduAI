...
test.describe("TA — canManageTopics policy edge cases", () => {
  ...
  test("policy ON then OFF: own-only carve-out survives the toggle for a topic the TA created while it was ON", async ({
    ...
    try {
      ...
      if (createdTopicId) {
        await adminCtx
          .delete(`${CORE_URL}/api/courses/${COURSES.cosc101}/topics/${createdTopicId}`, {
            data: { topicId: createdTopicId },
          })
          .catch(() => {});
      }
...
test.describe("Chat history — delete, switch, IDOR, rename", () => {
  ...
  test("deleting a nonexistent/already-deleted chat 404s (idempotent-safe)", async ({
    playwright,
  }) => {
    const ctx = await newAuthedContext(playwright, USERS.student1);
    try {
  ...
  test("IDOR: a TA cannot delete another user's chat (404, no existence leak)", async ({
    playwright,
  }) => {
...
test.describe("Bug report — UI submission", () => {
  test("TA submits a bug report through the dialog end-to-end, appears in their own list", async ({
    ...
    try {
      ...
      await expect(page.getByRole("heading", { name: "Report a bug" })).not.toBeVisible({
        timeout: 10_000,
      });
      ...
      expect(
        found,
        "submitted report should appear in GET /api/bug-reports?mine=true",
      ).toBeTruthy();
    } finally {
      await ctx.dispose();
    }
...
test.describe("Course chat — mocked deterministic reply", () => {
  for (const [label, email] of [
    ...
    test(`${label}: mocked assistant reply renders in the chat UI`, async ({
      page,
      playwright,
    }) => {
      ...
      try {
        ...
        await page.route("**/api/chat", async (route) => {
          ...
          await route.fulfill({
            ...
            body: buildMockStreamBody(
              "This is a deterministic round2 mocked reply about COSC 101.",
            ),
          });
        });
