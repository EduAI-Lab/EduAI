...
const OUT =
  process.env.PERF_OUT ?? `docs/perf/backend/baseline-${new Date().toISOString().slice(0, 10)}`;
...
const sh = (cmd) => {
  try {
    return stripAnsi(execSync(cmd + " 2>&1", { stdio: ["ignore", "pipe", "ignore"] }).toString());
  } catch (e) {
    return stripAnsi(e.stdout?.toString() ?? "");
  }
};
...
console.error(
  sh(
    `${BIN}/jscpd --output ${OUT}/duplication ${Object.values(SRC)
      .map((s) => s.path)
      .join(" ")}`,
  ),
);
...
for (const [name, { ext, path }] of Object.entries(SRC)) {
  ...
  circular += `\n=== ${name} (${path}) ===\n${out.trim()}\n`;
  writeFileSync(
    `${OUT}/dep-graph/graph-${name}.json`,
    shOut(`${BIN}/madge --json --extensions ${ext} ${path}`),
  );
}
...
sh(
  `cd apps/extensions/ai-tutor && ../../../${BIN}/knip --no-progress --no-exit-code > ../../../${OUT}/dead-code/knip-aitutor.txt 2>&1`,
);
sh(
  `cd apps/core && ../../${BIN}/knip --no-progress --no-exit-code > ../../${OUT}/dead-code/knip-core-unconfigured.txt 2>&1`,
);
sh(
  `cd apps/extensions/question-maker/app/backend && ../../../../../${BIN}/knip --no-progress --no-exit-code > ../../../../../${OUT}/dead-code/knip-qm-backend-unconfigured.txt 2>&1`,
);
sh(
  `cd apps/extensions/question-maker/app/frontend && ../../../../../${BIN}/knip --no-progress --no-exit-code > ../../../../../${OUT}/dead-code/knip-qm-frontend-unconfigured.txt 2>&1`,
);
