...
for (const a of after.results) {
  ...
  if (!b) continue;
  const d = (metric) => {
    const bv = b.latency_ms[metric],
      av = a.latency_ms[metric];
    if (bv == null || av == null) return { before: bv, after: av, delta: null, pct: null };
    return {
      before: bv,
      after: av,
      delta: round(av - bv),
      pct: bv ? round(((av - bv) / bv) * 100) : null,
    };
  };
  rows.push({
    key: key(a),
...
if (asMd) {
  console.log(
    `# Perf delta — ${before.env?.target_label} @ ${before.env?.git_sha} → ${after.env?.git_sha}\n`,
  );
  console.log("| App/Endpoint | p50 (b→a) | p95 (b→a) | p99 (b→a) | p95 Δ% |");
  console.log("|---|---|---|---|---|");
  for (const r of rows) {
    console.log(
      `| ${r.key} | ${r.p50.before}→${r.p50.after} | ${r.p95.before}→${r.p95.after} | ${r.p99.before}→${r.p99.after} | ${fmtPct(r.p95.pct)} |`,
    );
  }
} else {
  for (const r of rows) {
    console.log(
      `${r.key.padEnd(50)} p95 ${String(r.p95.before).padStart(7)}→${String(r.p95.after).padStart(7)}ms (${fmtPct(r.p95.pct)})`,
    );
  }
}
function fmtPct(p) {
  return p == null ? "n/a" : `${p > 0 ? "+" : ""}${p}%`;
}
