...
function poolDir() {
  ...
  for (let i = 0; i < 6; i++) {
    ...
    if (existsSync(pkg)) {
      try {
        if (JSON.parse(readFileSync(pkg, "utf8")).workspaces) return path.join(d, ".perf-pool");
      } catch {
        /* */
      }
    }
...
function summarize(samples) {
  ...
  for (const s of samples) {
    ...
    if (!isOk(s.status)) {
      ...
      if (!errSample)
        errSample = {
          status: s.status,
          message: s.errText ?? null,
          body: s.text ?? s.errText ?? null,
        };
    }
  }
  ...
  return {
    ...
    latency_ms: {
      ...
      mean: round(mean),
      min: round(ms[0] ?? null),
      max: round(ms.length ? ms[ms.length - 1] : null),
    },
    payload_bytes: {
      avg: bytes.length ? Math.round(bytes.reduce((a, b) => a + b, 0) / bytes.length) : null,
      p95: pct(bytes, 95),
      max: bytes.length ? bytes[bytes.length - 1] : null,
    },
    status_codes: statuses,
    error_rate: round(errors / (samples.length || 1)),
    error_sample: errSample,
  };
}
...
async function validateGate() {
  ...
  for (;;) {
    ...
    while (_validateHits.length && now - _validateHits[0] >= VALIDATE_WINDOW_MS)
      _validateHits.shift();
...
async function discover(cookies) {
  ...
  try {
    const r = await fetch(`${CORE_URL}/api/admin/cron-jobs`, {
  ...
  try {
    ...
    if (flags && typeof flags === "object") {
      const key = Object.keys(flags).find(
        (k) => typeof (flags[k]?.value ?? flags[k]) === "boolean",
      );
      if (key) {
        d.policyKey = key;
        d.policyOrig = flags[key]?.value ?? flags[key];
      }
    }
  } catch {
    /* */
...
function buildReads(cookies, m) {
  const t = [];
  const add = (app, method, path, role, url, pace = false) =>
    t.push({ app, method, path, role, url, kind: "read", pace });
  const C = m.core,
    A = m.aitutor,
    Q = m.qm;
  ...
  if (CORE_URL) {
    ...
    add(
      ...
      "/api/ai-providers",
      "admin",
      `${CORE_URL}/api/ai-providers?${CORE_PAGE_QUERY}`,
    );
    ...
    add(
      ...
      "student",
      `${CORE_URL}/api/sessions/validate`,
      true,
    );
    if (C) {
      ...
      add(
        ...
        "/api/courses/:id",
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}`,
      );
      add(
        "core",
        "GET",
        ...
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/materials`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/materials/:mid",
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/materials/${C.updateMaterialId}`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/embedding-settings",
        "instructor",
        `${CORE_URL}/api/courses/${C.updateCourseId}/embedding-settings`,
      );
      add(
        "core",
        "GET",
        ...
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/topics`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/topics/:tid",
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/topics/${C.readTopicId}`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/chats",
        "admin",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/chats`,
      );
      add(
        "core",
        "GET",
        "/api/units/:dept/chats",
        "admin",
        `${CORE_URL}/api/units/${C.department}/chats`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/tas",
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/tas`,
      );
      add(
        "core",
        "GET",
        ...
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/enrollments`,
      );
      add(
        "core",
        "GET",
        "/api/courses/:id/rag-settings",
        "instructor",
        `${CORE_URL}/api/courses/${C.sharedCourseId}/rag-settings`,
      );
      add(
        "core",
        "GET",
        ...
        "instructor",
        `${CORE_URL}/api/questions?courseId=${C.sharedCourseId}`,
      );
      add(
        "core",
        "GET",
        "/api/questions/:id",
        "instructor",
        `${CORE_URL}/api/questions/${C.readQuestionId}`,
      );
      ...
      add(
        ...
        "student",
        `${CORE_URL}/api/chats/${C.readChatId}/messages`,
      );
    }
  }
  ...
  if (AITUTOR_URL) {
    ...
    add(
      ...
      "student",
      `${AITUTOR_URL}/api/suggested-prompts`,
    );
    ...
    add(
      ...
      "admin",
      `${AITUTOR_URL}/api/admin/settings/eduai-api-key`,
    );
    add(
      "ai-tutor",
      "GET",
    ...
    if (A) {
      ...
      add(
        ...
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/feedback`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/submissions`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/student-metrics`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/analytics`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/modules`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/courses/:id/topics",
        "instructor",
        `${AITUTOR_URL}/api/courses/${c}/topics`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "admin",
        `${AITUTOR_URL}/api/admin/courses/${c}/enrollments`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/modules/:id",
        "instructor",
        `${AITUTOR_URL}/api/modules/${A.seededModuleId}`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/modules/${A.seededModuleId}/lessons`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/lessons/:id",
        "instructor",
        `${AITUTOR_URL}/api/lessons/${A.seededLessonId}`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/lessons/:id/activities",
        "instructor",
        `${AITUTOR_URL}/api/lessons/${A.seededLessonId}/activities`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/activities/:id/submissions",
        "instructor",
        `${AITUTOR_URL}/api/activities/${A.seededActivityId}/submissions`,
      );
      add(
        "ai-tutor",
        "GET",
        ...
        "instructor",
        `${AITUTOR_URL}/api/activities/${A.seededActivityId}/feedback`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/activities/:id/chat-sessions",
        "student",
        `${AITUTOR_URL}/api/activities/${A.seededActivityId}/chat-sessions`,
      );
      add(
        "ai-tutor",
        "GET",
        "/api/activities/:id/chat-sessions/:cid/messages",
        "student",
        `${AITUTOR_URL}/api/activities/${A.seededActivityId}/chat-sessions/${A.seededChatId}/messages`,
      );
    }
  }
  ...
  if (QM_URL) {
    ...
    if (Q) {
      ...
      add(
        "qm",
        "GET",
        "/api/course/:id/enrollments",
        "instructor",
        `${QM_URL}/api/course/${c}/enrollments`,
      );
      add(
        "qm",
        "GET",
        "/api/questions/export?courseId",
        "instructor",
        `${QM_URL}/api/questions/export?courseId=${c}&format=json`,
      );
      add(
        "qm",
        "GET",
        "/api/questions/:id",
        "instructor",
        `${QM_URL}/api/questions/${Q.anchorQuestionId}`,
      );
      add(
        "qm",
        "GET",
        "/api/questions/:id/variants",
        "instructor",
        `${QM_URL}/api/questions/${Q.anchorQuestionId}/variants`,
      );
      add(
        "qm",
        "GET",
        "/api/assessments/:id",
        "instructor",
        `${QM_URL}/api/assessments/${Q.anchorAssessmentId}`,
      );
      add(
        "qm",
        "GET",
        ...
        "instructor",
        `${QM_URL}/api/assessments/${Q.anchorAssessmentId}/questions`,
      );
      add(
        "qm",
        "GET",
        "/api/assessments/:id/sections",
        "instructor",
        `${QM_URL}/api/assessments/${Q.anchorAssessmentId}/sections`,
      );
      add(
        "qm",
        "GET",
        "/api/assessments/questions/:qid/check-in-assessments",
        "instructor",
        `${QM_URL}/api/assessments/questions/${Q.anchorQuestionId}/check-in-assessments`,
      );
      add(
        "qm",
        "GET",
        "/api/assessment-variant/assessments/:id/blueprint-snapshot",
        "instructor",
        `${QM_URL}/api/assessment-variant/assessments/${Q.anchorAssessmentId}/blueprint-snapshot`,
      );
      add(
        "qm",
        "GET",
        "/api/assessment-variant/assessments/:id/variant-readiness",
        "instructor",
        `${QM_URL}/api/assessment-variant/assessments/${Q.anchorAssessmentId}/variant-readiness`,
      );
      add(
        "qm",
        "GET",
        "/api/topics/sync-status/:id",
        "instructor",
        `${QM_URL}/api/topics/sync-status/${c}`,
      );
    }
  }
...
async function measureRead(tgt, cookies) {
  ...
  for (let i = 0; i < SAMPLES; i++)
    samples.push(await pacedFetch(tgt.app, tgt.url, opts, tgt.pace));
  return {
    app: tgt.app,
    method: tgt.method,
    path: tgt.path,
...
function opCreate({ app, path, role, url, body, idPath, cleanupUrl }) {
  return async (cookies) => {
    ...
    const created = [],
      samples = [];
    for (let i = 0; i < MUT; i++) {
      const s = await pacedFetch(app, url, {
        method: "POST",
        headers: H,
      ...
      samples.push(s);
      if (idPath) {
        const id = idFrom(s.text, idPath);
        if (id != null) created.push(id);
      }
    }
    if (cleanupUrl && created.length)
      for (const id of created)
        await pacedFetch(app, cleanupUrl(id), {
          method: "DELETE",
...
function opUpdate({ app, method, path, role, url, body, prepare }) {
  return async (cookies) => {
    ...
    for (let i = 0; i < MUT; i++)
      samples.push(
        await pacedFetch(app, url, { method, headers: H, body: JSON.stringify(body(i, ctx)) }),
      );
...
function opDelete({ app, method = "DELETE", path, role, pool, urlFn, body }) {
  return async (cookies) => {
    ...
    for (const v of list) {
      const opts = {
        method,
        headers: body ? jsonHeaders(cookies[role]) : { Cookie: cookies[role] ?? "" },
      };
    ...
    return list.length
      ? mutRow(app, method, path, role, samples, note)
      : errRow(app, method, path, role, "empty pool — run db:seed:perf");
  };
}
const errRow = (app, method, path, role, msg) => ({
  app,
  method,
  path,
  role,
  ...
  ok_count: 0,
  error_count: 1,
  latency_ms: { p50: null, p95: null, p99: null, mean: null, min: null, max: null },
  payload_bytes: { avg: null, p95: null, max: null },
  status_codes: {},
  error_rate: 1,
  error_sample: { status: 0, message: msg },
  note: "skipped",
});
  ...
  if (CORE_URL && C) {
    ...
    ops.push(
      opCreate({
        ...
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/courses/${id}`,
        ...
        body: (i) => ({
          ...
          isPublished: false,
          aiInstructions: "",
          instructorUserIds: [C.instructorUserId],
        }),
      }),
    );
    ops.push(
      opUpdate({
        ...
        role: "instructor",
        url: `${base}/api/courses/${C.updateCourseId}`,
        body: (i) => ({ name: `Perf C v${i}` }),
      }),
    );
    ops.push(
      opDelete({
        ...
        role: "instructor",
        pool: C.deleteCoursePool,
        urlFn: (id) => `${base}/api/courses/${id}`,
      }),
    );
    ops.push(
      opUpdate({
        ...
        role: "instructor",
        url: `${base}/api/courses/${C.updateCourseId}/publish`,
        body: () => ({}),
      }),
    );
    ops.push(
      opUpdate({
        ...
        role: "instructor",
        url: `${base}/api/courses/${C.updateCourseId}/unpublish`,
        body: () => ({}),
      }),
    );
    // materials (metadata)
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/courses/:id/materials/:mid",
        role: "instructor",
        url: `${base}/api/courses/${C.sharedCourseId}/materials/${C.updateMaterialId}`,
        body: (i) => ({ title: `Renamed ${i}` }),
      }),
    );
    ops.push(
      opUpdate({
        app: "core",
        method: "PUT",
        path: "PUT /api/courses/:id/materials/:mid",
        role: "instructor",
        url: `${base}/api/courses/${C.sharedCourseId}/materials/${C.updateMaterialId}`,
        body: (i) => ({ title: `Renamed put ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/courses/:id/materials/:mid",
        role: "instructor",
        pool: C.deleteMaterialPool,
        urlFn: (id) => `${base}/api/courses/${C.sharedCourseId}/materials/${id}`,
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/courses/:id/embedding-settings",
        role: "instructor",
        url: `${base}/api/courses/${C.updateCourseId}/embedding-settings`,
        body: () => ({ embeddingProvider: "local", embeddingModel: "mxbai-embed-large" }),
      }),
    );
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/courses/:id/rag-settings",
        role: "instructor",
        url: `${base}/api/courses/${C.updateCourseId}/rag-settings`,
        body: () => ({ ragTopK: 5, ragSimilarityThreshold: 0.5 }),
      }),
    );
    // topics
    ops.push(
      opCreate({
        ...
        role: "instructor",
        url: `${base}/api/courses/${C.sharedCourseId}/topics`,
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/courses/${C.sharedCourseId}/topics/${id}`,
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/courses/:id/topics/:tid",
        role: "instructor",
        url: `${base}/api/courses/${C.sharedCourseId}/topics/${C.updateTopicId}`,
        body: (i) => ({ name: `Renamed ${uniq("t", i)}` }),
      }),
    );
    ...
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/courses/:id/topics/:tid",
        role: "instructor",
        pool: C.deleteTopicPool,
        urlFn: (id) => `${base}/api/courses/${C.sharedCourseId}/topics/${id}`,
        body: (id) => ({ topicId: id }),
      }),
    );
    // TAs
    ops.push(
      opDelete({
        app: "core",
        method: "POST",
        path: "POST /api/courses/:id/tas",
        role: "instructor",
        pool: C.taAddUserPool,
        urlFn: () => `${base}/api/courses/${C.sharedCourseId}/tas`,
        body: (uid) => ({ userId: uid }),
      }),
    );
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/courses/:id/tas",
        role: "instructor",
        pool: C.taRemoveUserIds,
        urlFn: () => `${base}/api/courses/${C.sharedCourseId}/tas`,
        body: (uid) => ({ userId: uid }),
      }),
    );
    ...
    ops.push(
      opDelete({
        app: "core",
        method: "POST",
        path: "POST /api/courses/:id/enrollments",
        role: "instructor",
        pool: C.enrolleeUserPool,
        urlFn: () => `${base}/api/courses/${C.sharedCourseId}/enrollments`,
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/courses/:id/enrollments/:eid",
        role: "instructor",
        url: `${base}/api/courses/${C.sharedCourseId}/enrollments/${C.updateEnrollmentId}`,
        body: () => ({ role: "TA" }),
      }),
    );
    // questions
    ops.push(
      opCreate({
        app: "core",
        path: "POST /api/questions",
        role: "instructor",
        url: `${base}/api/questions`,
        idPath: "id",
        body: (i) => ({
          courseId: C.sharedCourseId,
          topicId: C.readTopicId,
          content: `Perf Q ${i}?`,
          type: "MCQ",
          choices: [
            { letter: "A", text: "x" },
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/questions/:id",
        role: "instructor",
        url: `${base}/api/questions/${C.questionUpdateId}`,
        body: (i) => ({ testable: i % 2 === 0 }),
      }),
    );
    // ai providers / models
    ops.push(
      opCreate({
        app: "core",
        path: "POST /api/ai-providers",
        role: "admin",
        url: `${base}/api/ai-providers`,
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/ai-providers/${id}`,
        body: (i) => ({
          name: uniq("perf-prov", i),
          displayName: "Perf",
          description: "perf",
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/ai-providers/:id",
        role: "admin",
        url: `${base}/api/ai-providers/${C.updateProviderId}`,
        body: (i) => ({ displayName: `Perf v${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/ai-providers/:id",
        role: "admin",
        pool: C.deleteProviderPool,
        urlFn: (id) => `${base}/api/ai-providers/${id}`,
      }),
    );
    ops.push(
      opCreate({
        app: "core",
        path: "POST /api/ai-models",
        role: "admin",
        url: `${base}/api/ai-models`,
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/ai-models/${id}`,
        body: (i) => ({
          modelId: uniq("perf-model", i),
          name: "Perf Model",
          description: "perf",
          type: "CHAT",
          providerId: C.updateProviderId,
          isActive: true,
        }),
      }),
    );
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/ai-models/:id",
        role: "admin",
        url: `${base}/api/ai-models/${C.updateModelId}`,
        body: (i) => ({ name: `Perf Model v${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/ai-models/:id",
        role: "admin",
        pool: C.deleteModelPool,
        urlFn: (id) => `${base}/api/ai-models/${id}`,
      }),
    );
    ...
    ops.push(
      opCreate({
        ...
        url: `${base}/api/users`,
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/users/${id}`,
        body: (i) => ({
          name: "Perf User",
          email: `perf+${uniq("u", i)}@perf.local`,
          role: "STUDENT",
        }),
      }),
    );
    ops.push(
      opUpdate({
        ...
        role: "admin",
        url: `${base}/api/users/${C.updateUserId}`,
        body: (i) => ({ name: `Perf User v${i}` }),
      }),
    );
    ops.push(
      opDelete({
        ...
        role: "admin",
        pool: C.deleteUserPool,
        urlFn: (id) => `${base}/api/users/${id}`,
      }),
    );
    // invitations (admin)
    ...
    ops.push(
      opCreate({
        app: "core",
        path: "POST /api/invitations",
        role: "admin",
        url: `${base}/api/invitations`,
        idPath: "invitation.id",
        cleanupUrl: (id) => `${base}/api/invitations/${id}`,
        body: (i) => ({ email: `invite-${uniq("i", i)}@ubc.ca`, role: "INSTRUCTOR" }),
      }),
    );
    ops.push(
      opUpdate({
        app: "core",
        method: "POST",
        path: "POST /api/invitations/:id (resend)",
        role: "admin",
        url: `${base}/api/invitations/${C.resendInvitationId}`,
        body: () => ({}),
      }),
    );
    ops.push(
      opDelete({
        app: "core",
        path: "DELETE /api/invitations/:id",
        role: "admin",
        pool: C.deleteInvitationPool,
        urlFn: (id) => `${base}/api/invitations/${id}`,
      }),
    );
    // bug reports
    ops.push(
      opUpdate({
        app: "core",
        method: "POST",
        path: "POST /api/bug-reports",
        role: "actor",
        url: `${base}/api/bug-reports`,
        body: (i) => ({ description: `perf bug ${i}` }),
      }),
    );
    ops.push(
      opUpdate({
        app: "core",
        method: "PATCH",
        path: "PATCH /api/admin/bug-reports/:id",
        role: "admin",
        url: `${base}/api/admin/bug-reports/${C.bugReportId}`,
        body: (i) => ({ status: i % 2 ? "IN_PROGRESS" : "UNHANDLED" }),
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "core",
        method: "POST",
        path: "POST /api/assistive-events",
        role: "actor",
        url: `${base}/api/assistive-events`,
        body: () => ({ eventType: "mode_toggled" }),
      }),
    );
    ...
    if (disc.cronJob)
      ops.push(
        opUpdate({
          app: "core",
          method: "POST",
          path: "POST /api/admin/cron-jobs",
          role: "admin",
          url: `${base}/api/admin/cron-jobs`,
          body: () => ({
            intent: "update-schedule",
            jobName: disc.cronJob,
            schedule: "0 3 * * *",
            scheduleLabel: "Daily 3am",
          }),
        }),
      );
    ...
    if (disc.policyKey) {
      ops.push(
        opUpdate({
          app: "core",
          method: "PATCH",
          path: "PATCH /api/policies",
          role: "admin",
          url: `${base}/api/policies`,
          body: () => ({ key: disc.policyKey, value: true }),
        }),
      );
      ops.push(
        opUpdate({
  ...
  if (AITUTOR_URL && A) {
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/modules/:id/publish",
        role: "instructor",
        url: `${base}/api/modules/${A.poolModulesReuse[0]}/publish`,
        body: () => ({}),
      }),
    );
    ops.push(
      opDelete({
        app: "ai-tutor",
        path: "DELETE /api/modules/:id",
        role: "instructor",
        pool: A.poolModulesDrop,
        urlFn: (id) => `${base}/api/modules/${id}`,
      }),
    );
    ops.push(
      opCreate({
        app: "ai-tutor",
        path: "POST /api/modules/:id/lessons",
        role: "instructor",
        url: `${base}/api/modules/${A.poolModulesReuse[1]}/lessons`,
        idPath: "id",
        cleanupUrl: (id) => `${base}/api/lessons/${id}`,
        body: (i) => ({ title: `PERF les ${i}`, position: 0 }),
      }),
    );
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/lessons/:id/publish",
        role: "instructor",
        url: `${base}/api/lessons/${A.poolLessonsReuse[0]}/publish`,
        body: () => ({}),
      }),
    );
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/lessons/:id/unpublish",
        role: "instructor",
        url: `${base}/api/lessons/${A.poolLessonsReuse[0]}/unpublish`,
        body: () => ({}),
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/modules/:id/unpublish",
        role: "instructor",
        url: `${base}/api/modules/${A.poolModulesReuse[0]}/unpublish`,
        body: () => ({}),
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/lessons/:id",
        role: "instructor",
        url: `${base}/api/lessons/${A.poolLessonsReuse[1]}`,
        body: (i) => ({ contentMd: `PERF ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "ai-tutor",
        path: "DELETE /api/lessons/:id",
        role: "instructor",
        pool: A.poolLessonsDrop,
        urlFn: (id) => `${base}/api/lessons/${id}`,
      }),
    );
    ops.push(
      opCreate({
        app: "ai-tutor",
        path: "POST /api/lessons/:id/activities",
        role: "instructor",
        url: `${base}/api/lessons/${A.poolLessonsReuse[0]}/activities`,
        idPath: "id",
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PATCH",
        path: "PATCH /api/activities/:id",
        role: "instructor",
        url: `${base}/api/activities/${A.poolActivitiesReuse[0]}`,
        body: (i) => ({ title: `PERF ${i}` }),
      }),
    );
    ops.push(
    ...
    ops.push(
      opCreate({
        app: "ai-tutor",
        path: "POST /api/prompts",
        role: "instructor",
        url: `${base}/api/prompts`,
        idPath: "id",
        body: (i) => ({
          name: `PERF prompt ${uniq("p", i)}`,
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PUT",
        path: "PUT /api/admin/settings/eduai-api-key",
        role: "admin",
        url: `${base}/api/admin/settings/eduai-api-key`,
        body: () => ({ apiKey: "perf-test-key" }),
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "ai-tutor",
        method: "PUT",
        path: "PUT /api/admin/settings/ai-model-policy",
        role: "admin",
        url: `${base}/api/admin/settings/ai-model-policy`,
        prepare: async (cookies) => {
          const r = await pacedFetch("ai-tutor", `${base}/api/admin/settings/ai-model-policy`, {
            headers: { Cookie: cookies.admin ?? "" },
          });
  ...
  if (QM_URL && Q) {
    ...
    ops.push(
      opCreate({
        ...
        url: `${base}/api/questions`,
        idPath: "data.id",
        cleanupUrl: (id) => `${base}/api/questions/${id}`,
        body: () => ({ courseId: c, primaryTopicId: tid, type: "MCQ", description: "perf q" }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/questions/:id",
        role: "instructor",
        url: `${base}/api/questions/${Q.questionUpdateId}`,
        body: (i) => ({ description: `perf upd ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/questions/:id",
        role: "instructor",
        pool: Q.questionDeletePool,
        urlFn: (id) => `${base}/api/questions/${id}`,
      }),
    );
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/questions/approve",
        role: "instructor",
        url: `${base}/api/questions/approve`,
        body: () => ({
          courseId: c,
          questions: [{ description: "perf approved", primaryTopicId: tid, type: "MCQ" }],
        }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/questions/:id/order",
        role: "instructor",
        url: `${base}/api/questions/${Q.anchorQuestionId}/order`,
        body: () => ({ assessmentId: Q.anchorAssessmentId, orderNumber: 1 }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/questions/:id/order/:aid",
        role: "instructor",
        pool: Q.questionOrderPool,
        urlFn: (p) => `${base}/api/questions/${p.questionId}/order/${p.assessmentId}`,
      }),
    );
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/questions/:id/variants",
        role: "instructor",
        url: `${base}/api/questions/${Q.anchorQuestionId}/variants`,
        idPath: "data.id",
        cleanupUrl: (id) => `${base}/api/questions/variants/${id}`,
        body: (i) => ({
          questionText: `perf variant ${i}`,
          difficulty: "medium",
    ...
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/questions/variants/:id",
        role: "instructor",
        url: `${base}/api/questions/variants/${Q.variantUpdateId}`,
        body: (i) => ({ questionText: `perf upd variant ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/questions/variants/:id",
        role: "instructor",
        pool: Q.variantDeletePool,
        urlFn: (id) => `${base}/api/questions/variants/${id}`,
      }),
    );
    // course
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/course",
        role: "instructor",
        url: `${base}/api/course`,
        idPath: "data.id",
        cleanupUrl: (id) => `${base}/api/course/${id}`,
        body: (i) => ({ name: `__PERF__ new ${uniq("c", i)}`, courseCode: "PERFN" }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/course/:id",
        role: "instructor",
        url: `${base}/api/course/${Q.courseUpdateId}`,
        body: (i) => ({ name: `__PERF__ upd ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/course/:id",
        role: "instructor",
        pool: Q.courseDeletePool,
        urlFn: (id) => `${base}/api/course/${id}`,
      }),
    );
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/course/:id/topics",
        role: "instructor",
        url: `${base}/api/course/${c}/topics`,
        idPath: "data.id",
        body: (i) => ({ name: `__PERF__ topic ${uniq("t", i)}` }),
      }),
    );
    // assessments
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/assessments",
        role: "instructor",
        url: `${base}/api/assessments`,
        idPath: "data.id",
        cleanupUrl: (id) => `${base}/api/assessments/${id}`,
        body: () => ({ type: "Quiz", name: "__PERF__ a", semester: "Fall 2026", courseId: c }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/assessments/:id",
        role: "instructor",
        url: `${base}/api/assessments/${Q.assessmentUpdateId}`,
        body: (i) => ({ name: `__PERF__ upd a ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/assessments/:id",
        role: "instructor",
        pool: Q.assessmentDeletePool,
        urlFn: (id) => `${base}/api/assessments/${id}`,
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "POST",
        path: "POST /api/assessments/:id/questions",
        role: "instructor",
        url: `${base}/api/assessments/${Q.anchorAssessmentId}/questions`,
        body: () => ({ questionId: Q.anchorQuestionId, orderNumber: 1 }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/assessments/:id/questions/:qid",
        role: "instructor",
        pool: Q.questionOrderPool,
        urlFn: (p) => `${base}/api/assessments/${p.assessmentId}/questions/${p.questionId}`,
      }),
    );
    // sections
    ops.push(
      opCreate({
        app: "qm",
        path: "POST /api/assessments/:id/sections",
        role: "instructor",
        url: `${base}/api/assessments/${Q.anchorAssessmentId}/sections`,
        idPath: "data.id",
        cleanupUrl: (id) => `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${id}`,
        body: (i) => ({ name: `__PERF__ sec ${i}` }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/assessments/:aid/sections/:sid",
        role: "instructor",
        url: `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${Q.sectionUpdateId}`,
        body: (i) => ({ name: `__PERF__ upd sec ${i}` }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/assessments/:aid/sections/:sid",
        role: "instructor",
        pool: Q.sectionDeletePool,
        urlFn: (id) => `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${id}`,
      }),
    );
    // section variants
    ...
    ops.push(
      opDelete({
        app: "qm",
        method: "POST",
        path: "POST /api/assessments/:aid/sections/:sid/variants",
        role: "instructor",
        pool: Q.sectionVariantAddPool,
        urlFn: () =>
          `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${Q.anchorSectionId}/variants`,
        body: (vid) => ({ variantId: vid, displayOrder: 1 }),
      }),
    );
    ops.push(
      opUpdate({
        app: "qm",
        method: "PUT",
        path: "PUT /api/assessments/:aid/sections/:sid/variants/:vid/order",
        role: "instructor",
        url: `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${Q.sectionVariantUpdate.sectionId}/variants/${Q.sectionVariantUpdate.variantId}/order`,
        body: (i) => ({ displayOrder: i + 1 }),
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/assessments/:aid/sections/:sid/variants/:vid",
        role: "instructor",
        pool: Q.sectionVariantDeletePool,
        urlFn: (p) =>
          `${base}/api/assessments/${Q.anchorAssessmentId}/sections/${p.sectionId}/variants/${p.variantId}`,
      }),
    );
    ops.push(
      opDelete({
        app: "qm",
        path: "DELETE /api/assessments/questions/:qid/remove-from-all-sections",
        role: "instructor",
        pool: Q.questionWithSectionLinkPool,
        urlFn: (qid) => `${base}/api/assessments/questions/${qid}/remove-from-all-sections`,
      }),
    );
    ...
    ops.push(
      opUpdate({
        app: "qm",
        method: "PATCH",
        path: "PATCH /api/assessment-variant/assessments/:id/role",
        role: "instructor",
        url: `${base}/api/assessment-variant/assessments/${Q.anchorAssessmentId}/role`,
        body: (i) => ({ studyRole: i % 2 ? "reference_baseline" : "generated_variant" }),
      }),
    );
  }
...
function preflight(m) {
  ...
  if (problems.length) {
    ...
    console.error(
      "  Run `npm run db:seed:perf` (seeds all three DBs + writes the manifests), then retry.",
    );
    return false;
  }
  // Warn (not fatal) on depleted delete pools.
  const low = [];
  const chk = (obj, keys, app) =>
    obj &&
    keys.forEach((k) => {
      if (Array.isArray(obj[k]) && obj[k].length < MUT) low.push(`${app}.${k}=${obj[k].length}`);
    });
  chk(
    m.core,
    [
  ...
  if (low.length)
    console.error(
      `  ⚠ pools below PERF_MUT_SAMPLES=${MUT} (deletes measure fewer samples): ${low.join(" ")}\n    re-run db:seed:perf to refill.`,
    );
  return true;
}
...
function fingerprint() {
  ...
  return {
    ...
    mut_samples: MUT,
    concurrency: 1,
    include_mutations: RUN_MUTATIONS,
    validate_governor: {
      limit: VALIDATE_LIMIT,
      window_ms: VALIDATE_WINDOW_MS,
      applies_to: ["ai-tutor", "qm", "core:sessions/validate"],
    },
    seed_note:
      "Run `npm run db:seed:perf` (all three DBs + pool manifests) before each run; deletes consume the pool.",
  };
}
...
async function main() {
  ...
  const m = {
    core: loadManifest("core"),
    aitutor: loadManifest("aitutor"),
    qm: loadManifest("qm"),
  };
  ...
  if (RUN_MUTATIONS) {
    ...
    for (const op of ops) {
      ...
      try {
        ...
        r = {
          ...
          role: "?",
          kind: "mutation",
          ...summarize([{ ms: 0, status: 0, errText: String(e?.message ?? e) }]),
        };
      }
      results.push(r);
      logRow(r);
    ...
    if (disc.policyKey && cookies.admin) {
      try {
        await fetch(`${CORE_URL}/api/policies`, {
          method: "PATCH",
          headers: jsonHeaders(cookies.admin),
          body: JSON.stringify({ key: disc.policyKey, value: disc.policyOrig }),
        });
      } catch {
        /* */
      }
  ...
  const payload = {
    env: fingerprint(),
    pool_manifests: { core: !!m.core, aitutor: !!m.aitutor, qm: !!m.qm },
    results,
  };
  ...
  const slow = [...measured]
    .sort((a, b) => (b.latency_ms.p95 ?? 0) - (a.latency_ms.p95 ?? 0))
    .slice(0, 10);
  ...
  if (errored.length) {
    ...
    for (const r of errored)
      console.error(
        `  ✗ ${r.app} ${r.method} ${r.path} → ${r.error_sample?.status} ${r.error_sample?.message ?? ""}`,
      );
  }
}
...
function writeErrorLog(results, dir) {
  ...
  for (const r of errored) {
    ...
    lines.push(
      `  samples=${r.count ?? 0} errors=${r.error_count ?? 0} status_codes=${JSON.stringify(r.status_codes ?? {})}${r.note ? ` note=${r.note}` : ""}`,
    );
...
function logRow(r) {
  ...
  if (!r.ok_count)
    console.error(
      `  ${String(r.app).padEnd(8)} ${String(r.method).padEnd(6)} ${String(r.path).padEnd(46)} ⚠ ${r.error_sample?.status ?? "?"} ${r.error_sample?.message ?? ""}`.slice(
        0,
        160,
      ),
    );
    ...
    console.error(
      `  ${String(r.app).padEnd(8)} ${String(r.method).padEnd(6)} ${String(r.path).padEnd(46)} p50=${String(l.p50).padStart(6)} p95=${String(l.p95).padStart(6)}ms ${r.payload_bytes?.avg ?? 0}B [${Object.keys(r.status_codes).join(",")}]${r.error_count ? ` ⚠${r.error_count}` : ""}${r.note ? " " + r.note : ""}`,
    );
}
