import { Prisma, PrismaClient } from "@eduai/ai-tutor-prisma-client";
          ...
          const created = await prisma.activity.create({
            data: {
              ...
              secondaryTopics: secondaryIds.length
                ? {
                    create: secondaryIds.map((topicId) => ({
      ...
      data: {
        ...
        trace: {
          attempts: [
            ...
            {
              tutorDraft:
                "Let's break the problem into smaller parts. What is the first sub-problem?",
...
async function main() {
  ...
  console.log(`  ${offerings.length} course offerings linked to Core (coreOfferingId set)`);
