...
import type { Route } from '../../routes/+types/instructor.list';
...
vi.mock('~/hooks/useAtPermissions', () => ({
  useAtPermissions: () => ({ canManageContent: true }),
}));
...
import InstructorLessonBuilder from '~/routes/instructor.list';

const lesson = { id: 1, title: 'Lesson 1', moduleId: null };
const activity = {
  id: 99,
  title: 'Activity 1',
  instructionsMd: '',
...
function wrap(activities = [activity]) {
  const props = { loaderData: { lesson, activities, module: null, course: null } } as Route.ComponentProps;
  return render(
    <MemoryRouter>
...
describe('instructor.list — delete activity confirmation dialog', () => {
  beforeEach(() => {
  ...
  it('confirming calls deleteActivity with the activity id', async () => {
    ...
    await act(async () => {
      fireEvent.click(screen.getByRole('button', { name: /^remove$/i, hidden: false }));
    });
