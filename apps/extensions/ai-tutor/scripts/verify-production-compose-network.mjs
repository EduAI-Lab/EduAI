...
const rendered = spawnSync("docker", ["compose", "-f", composeFile, "config", "--format", "json"], {
  cwd: appDir,
  env: process.env,
  encoding: "utf8",
});
...
if (database.network_mode === "host")
  throw new Error("AI Tutor database must not use host networking");
...
if (postgresPort.host_ip !== "127.0.0.1" || String(postgresPort.published) !== "54321") {
