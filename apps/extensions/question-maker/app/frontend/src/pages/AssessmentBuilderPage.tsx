...
const AssessmentBuilderPage = () => {
  ...
  const hasQuestions = useMemo(() => {
    if (!assessment?.sections) return false;
    const count = assessment.sections.reduce((acc, s) => acc + (s.sectionVariants?.length ?? 0), 0);
    return count > 0;
  }, [assessment?.sections]);
    ...
    try {
      ...
      if (viewEntry?.questionId === questionId) {
        const resolveTopicName = (topicId: string) =>
          topicById[topicId]?.name ?? `Topic ${topicId}`;
        const variant =
          fetched.variants?.find((v) => v.id === viewEntry.variant.id) ?? viewEntry.variant;
        const secondaryTopicNames = Array.isArray(variant.secondaryTopicsId)
          ? (variant.secondaryTopicsId
  ...
  const handleQuestionCreated = (newQuestion: Question) => {
    ...
    void (async () => {
      ...
      toast("Question saved", {
        description:
          "The question and its variants have been updated. You can now add it to sections.",
      });
    })();
  };
  ...
  return (
    <>
      <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-4 md:py-6 lg:px-6">
            ...
            <div className="min-w-0 space-y-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                ...
                {assessment.course?.name && (
                  <Badge variant="outline">{assessment.course.name}</Badge>
                )}
              </div>
          ...
          <div className="flex shrink-0 flex-nowrap items-center gap-2">
            ...
            <PermissionGate allow={canManageAssessment}>
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem
                    onSelect={() => setTimeout(() => setIsEditAssessmentOpen(true), 0)}
                  >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatTile
            icon={IconLayoutList}
            label="Sections"
            value={assessmentStats.sectionCount}
            tone="secondary"
          />
          <StatTile
            icon={IconListCheck}
          ...
          <StatTile
            ...
            label="Reviewed"
            value={assessmentStats.reviewed}
            tone="success"
          />
          <StatTile
        ...
        <div className="rounded-[var(--radius-xl)] border border-border bg-card p-5 shadow-[var(--shadow-2xs)]">
          <AssessmentBuilder
            ...
            onRemoveQuestionFromSection={async (sectionId, variantId) => {
              if (!assessment) return;
              try {
                await assessmentService.removeVariantFromSection(
                  assessment.id,
                  sectionId,
                  variantId,
                );
      ...
      <QuestionModal
        ...
        relatedVariants={
          viewEntry
            ? questionVariantEntries.filter((e) => e.questionId === viewEntry.questionId)
            : []
        }
