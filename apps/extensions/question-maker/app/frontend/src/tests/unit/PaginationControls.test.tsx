...
describe("PaginationControls", () => {
  ...
  it("shows the current range and page count", () => {
    render(<PaginationControls page={2} pageSize={25} total={57} onPageChange={vi.fn()} />);
  ...
  it("clamps the final page range to total", () => {
    render(<PaginationControls page={3} pageSize={25} total={57} onPageChange={vi.fn()} />);
    expect(screen.getByText("Showing 51–57 of 57")).toBeInTheDocument();
  });

  it("disables Previous on the first page", () => {
    render(<PaginationControls page={1} pageSize={25} total={57} onPageChange={vi.fn()} />);
  ...
  it("fires onPageChange with the next/previous page", () => {
    ...
    render(<PaginationControls page={2} pageSize={25} total={57} onPageChange={onPageChange} />);
  ...
  it("notifies the parent when the requested page is past the end", () => {
    ...
    const onPageChange = vi.fn();
    render(<PaginationControls page={9} pageSize={25} total={57} onPageChange={onPageChange} />);
    expect(onPageChange).toHaveBeenCalledWith(3);
  });

  it("notifies the parent when the requested page is below 1", () => {
    const onPageChange = vi.fn();
    render(<PaginationControls page={0} pageSize={25} total={57} onPageChange={onPageChange} />);
  ...
  it("notifies the parent when the list empties out", () => {
    const onPageChange = vi.fn();
    render(<PaginationControls page={4} pageSize={25} total={0} onPageChange={onPageChange} />);
    expect(onPageChange).toHaveBeenCalledWith(1);
  });
  ...
  it("disables both controls when disabled", () => {
    render(
      <PaginationControls page={2} pageSize={25} total={57} onPageChange={vi.fn()} disabled />,
    );
    expect(screen.getByRole("button", { name: /previous/i })).toBeDisabled();
    expect(screen.getByRole("button", { name: /next/i })).toBeDisabled();
  });
