...
describe('QuestionBank', () => {
  ...
  it('shows the no-match empty state and clears filters on demand', () => {
    ...
    fireEvent.change(screen.getByLabelText('Search questions'), { target: { value: 'nonexistent' } });
  ...
  it('sorts by type when the sort control changes', () => {
    const entries = [
      makeEntry({ questionType: 'SA', variant: { ...makeEntry().variant, id: 1, questionText: 'SA question', choices: null } }),
      makeEntry({ questionId: 2, questionType: 'MCQ', variant: { ...makeEntry().variant, id: 2, questionText: 'MCQ question' } }),
    ];
    render(<QuestionBank variants={entries} {...baseProps()} />);
  ...
  it('hides the upload/add header buttons when disabled', () => {
    render(
      <QuestionBank variants={[makeEntry()]} {...baseProps()} disableAdd disableUpload />,
    );
  ...
  it('numbers non-base variants of the same question in creation order', () => {
    ...
    const variantA = makeEntry({
      variant: { ...makeEntry().variant, id: 2, referenceId: 1, questionText: 'Variant A text', createdAt: '2026-01-02T00:00:00.000Z' },
    });
    const variantB = makeEntry({
      variant: { ...makeEntry().variant, id: 3, referenceId: 1, questionText: 'Variant B text', createdAt: '2026-01-03T00:00:00.000Z' },
    });
