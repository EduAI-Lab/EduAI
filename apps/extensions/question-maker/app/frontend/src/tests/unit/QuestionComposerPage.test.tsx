...
vi.mock('@/components/questions/QuestionAIControls', () => ({
  QuestionAIControls: (props: any) => (
    <div data-testid="ai-controls">
      ...
      <button onClick={() => props.onChange('generationPrompt', 'a great prompt')}>set-prompt</button>
...
describe('QuestionComposerPage gate states', () => {
  ...
  it('shows a skeleton while access is loading', () => {
    useQmPermissionsForCourseMock.mockReturnValue({ canCreateQuestion: false, hasCourseAccess: false, accessLoading: true });
    render(<QuestionComposerPage />);
    expect(screen.getAllByTestId('skeleton').length).toBeGreaterThan(0);
  });

  it('shows a read-only message when the user cannot write', () => {
    useQmPermissionsForCourseMock.mockReturnValue({ canCreateQuestion: false, hasCourseAccess: true, accessLoading: false });
    render(<QuestionComposerPage />);
  ...
  it('shows a source-load error (edit mode)', async () => {
    routeParams.current = { courseId: '5', questionId: '42' };
    questionService.getQuestion.mockRejectedValue({ response: { data: { error: 'question gone' } } });
    render(<QuestionComposerPage />);
...
describe('QuestionComposerPage create mode', () => {
  ...
  it('creates a question + variant on valid save', async () => {
    ...
    await waitFor(() => expect(questionService.createVariant).toHaveBeenCalledWith(99, expect.objectContaining({ isDraft: true })));
    expect(navigateMock).toHaveBeenCalledWith('/courses/5?tab=questions');
  });
  ...
  it('marks the created variant reviewed when the checkbox is checked', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /save question/i }));
    await waitFor(() =>
      expect(questionService.createVariant).toHaveBeenCalledWith(99, expect.objectContaining({ isDraft: false })),
    );
  });

  it('reports a save failure, translating the VARIANT_LOCKED code', async () => {
    questionService.createQuestion.mockRejectedValue({ response: { data: { error: 'VARIANT_LOCKED' } } });
    render(<QuestionComposerPage />);
    await screen.findByText('New question');
    fireEvent.click(screen.getByText('set-text'));
    fireEvent.click(screen.getByText('set-choices'));
  ...
  it('switches question type via the selector', async () => {
    ...
    questionService.createVariant.mockResolvedValue({ id: 1 });
    fireEvent.click(screen.getByRole('button', { name: /save question/i }));
    await waitFor(() => expect(questionService.createQuestion).toHaveBeenCalledWith(expect.objectContaining({ type: 'SA' })));
  });
...
describe('QuestionComposerPage variant mode', () => {
  beforeEach(() => {
    ...
    questionService.getQuestion.mockResolvedValue({
      ...
      primaryTopicId: 't1',
      variants: [
        { id: 70, questionText: 'Base text', difficulty: 'easy', answer: 'A', choices: [{ letter: 'A', text: 'x' }], referenceId: null },
      ],
    });
  });
  ...
  it('creates a variant referencing the source question', async () => {
    ...
    await waitFor(() =>
      expect(questionService.createVariant).toHaveBeenCalledWith(7, expect.objectContaining({ referenceId: 70 })),
    );
    expect(navigateMock).toHaveBeenCalledWith('/courses/5?tab=questions');
  });
});
...
describe('QuestionComposerPage edit mode', () => {
  ...
  it('reopens an approved variant as a draft on edit', async () => {
    questionService.getQuestion.mockResolvedValue({
      ...
      primaryTopicId: 't1',
      variants: [
        { id: 70, questionText: 'Base text', difficulty: 'easy', answer: 'A', choices: [{ letter: 'A', text: 'x' }, { letter: 'B', text: 'y' }], isDraft: false },
      ],
    });
    ...
    await waitFor(() =>
      expect(questionService.updateVariant).toHaveBeenCalledWith(70, expect.objectContaining({ isDraft: true })),
    );
  ...
  it('lists other variants when more than one exists', async () => {
    questionService.getQuestion.mockResolvedValue({
      ...
      primaryTopicId: 't1',
      variants: [
        { id: 70, questionText: 'Variant A', difficulty: 'easy', answer: 'A', choices: [{ letter: 'A', text: 'x' }, { letter: 'B', text: 'y' }], isDraft: true },
...
describe('QuestionComposerPage AI generation', () => {
  ...
  it('generates a question and fills the form', async () => {
    ...
    await waitFor(() => expect(toastFn).toHaveBeenCalledWith('Question generated', expect.anything()));
  });

  it('surfaces a generation failure with error details', async () => {
    eduaiService.generateQuestions.mockRejectedValue({ response: { data: { aiErrorReason: 'quota exceeded' } } });
    render(<QuestionComposerPage />);
    await screen.findByText('New question');
    fireEvent.click(screen.getByText('set-prompt'));
    fireEvent.click(screen.getByText('generate'));
