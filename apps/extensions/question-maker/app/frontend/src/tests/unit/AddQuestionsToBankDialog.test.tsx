...
describe('AddQuestionsToBankDialog', () => {
  ...
  it('loads course questions, excludes bank members, and adds a selection', async () => {
    questionService.getQuestionsPage
      .mockResolvedValueOnce({ items: [makeQuestion(1, 'What is 2+2?'), makeQuestion(2, 'Capital of France')] })
    ...
    await waitFor(() => expect(questionBankService.addQuestionToBank).toHaveBeenCalledWith(7, 'bank-1', 1));
  ...
  it('filters the list by search text', async () => {
    ...
    render(
      <AddQuestionsToBankDialog open onClose={vi.fn()} courseId={7} bankId="bank-1" />,
    );
  ...
  it('reports failure when every add call rejects', async () => {
    ...
    await waitFor(() => expect(toastFn.error).toHaveBeenCalledWith('Could not add questions', expect.objectContaining({})));
  ...
  it('closes via onOpenChange when dismissed', async () => {
    questionService.getQuestionsPage.mockResolvedValueOnce({ items: [] }).mockResolvedValueOnce({ items: [] });
    const onClose = vi.fn();
    render(<AddQuestionsToBankDialog open onClose={onClose} courseId={7} bankId="bank-1" />);
