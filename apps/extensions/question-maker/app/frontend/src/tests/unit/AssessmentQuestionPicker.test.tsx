...
function renderPicker(props: Partial<React.ComponentProps<typeof AssessmentQuestionPicker>> = {}) {
  return render(
    <AssessmentQuestionPicker
      open
      onOpenChange={vi.fn()}
      questionBank={[entry(1), entry(2, { variant: { id: 2, questionText: 'Recursion basics', difficulty: 'hard' } })]}
      excludeVariantIds={[]}
      topics={topics}
      onConfirm={vi.fn()}
      {...props}
...
describe('AssessmentQuestionPicker', () => {
  ...
  it('filters by question type', () => {
    renderPicker({
      questionBank: [entry(1), entry(2, { questionType: 'SA', variant: { id: 2, questionText: 'SA question', difficulty: 'medium' } })],
    });
