...
vi.mock("@/services/assessmentVariantService", () => ({
  default: assessmentVariantService,
  assessmentVariantService,
}));
  ...
  CanvasImportDialog: (props: any) =>
    props.open ? (
      <div data-testid="canvas-import-dialog">
        ...
        <button
          onClick={() =>
            props.onImportSuccess({ assessmentId: 66, assessmentName: "Imported exam" })
          }
        >
          import-success
        </button>
...
function baselineAssessment(overrides: Partial<any> = {}) {
  return {
    ...
    sections: [
      {
        position: 0,
        sectionVariants: [{ displayOrder: 0, variant: { questionMetadataId: 1 } }],
      },
    ],
    ...overrides,
  };
...
describe("AssessmentVariantPage — course + baseline step", () => {
  it("renders the header and baseline step by default", async () => {
    ...
    await waitFor(() =>
      expect(assessmentService.getAssessments).toHaveBeenCalledWith({ courseId: 5 }),
    );
  ...
  it("preselects the baseline from the route param", async () => {
    ...
    render(<AssessmentVariantPage />);
    await waitFor(() =>
      expect(assessmentVariantService.getBaselineVariantReadiness).toHaveBeenCalledWith(10, 5),
    );
  });
  ...
  it("marks the baseline as the reference exam", async () => {
    ...
    await waitFor(() =>
      expect(assessmentVariantService.setStudyRole).toHaveBeenCalledWith(10, "reference_baseline"),
    );
  });

  it("reports a failure marking the baseline", async () => {
    assessmentVariantService.setStudyRole.mockRejectedValue({
  ...
  it("opens the OCR upload dialog and saves an exam as the new baseline", async () => {
    ...
    await waitFor(() =>
      expect(toastFn).toHaveBeenCalledWith("Baseline exam saved", expect.anything()),
    );
  });
  ...
  it("opens the Canvas import dialog and adopts the imported exam as baseline", async () => {
    ...
    await waitFor(() =>
      expect(toastFn).toHaveBeenCalledWith("Imported from Canvas", expect.anything()),
    );
  });
});
...
describe("AssessmentVariantPage — assemble step", () => {
  it("assembles a variant exam and lists it", async () => {
    ...
    await waitFor(() =>
      expect(assessmentVariantService.assembleEquivalentExams).toHaveBeenCalled(),
    );
  ...
  it("reports an assembly failure", async () => {
    assessmentVariantService.assembleEquivalentExams.mockRejectedValue({
    ...
    fireEvent.click(screen.getByRole("button", { name: /assemble variant exam/i }));
    await waitFor(() =>
      expect(toastFn.error).toHaveBeenCalledWith("Assembly failed", expect.anything()),
    );
  });
});
...
describe("AssessmentVariantPage — AI review step", () => {
  ...
  it("reports an AI review failure", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /run ai review/i }));
    await waitFor(() =>
      expect(toastFn.error).toHaveBeenCalledWith("AI review failed", expect.anything()),
    );
  });
  ...
  it("opens history, is empty by default, and closes", async () => {
    ...
    expect(
      await screen.findByText("No previous AI reviews for this course yet."),
    ).toBeInTheDocument();
  });
});
