...
import { getOCRHistoryStorageKey, MAX_HISTORY_ITEMS, MAX_STORED_QUESTIONS_PER_JOB } from '@/types/ocr';
...
describe('useOCRHistory', () => {
  ...
  it('loads jobs already present in localStorage', async () => {
    const stored: OCRJob[] = [
      { id: 'a', createdAt: new Date().toISOString(), fileName: 'x.pdf', status: 'success', courseId: 1 } as OCRJob,
    ];
  ...
  it('addJob caps storedQuestions at MAX_STORED_QUESTIONS_PER_JOB', async () => {
    ...
    const many = Array.from({ length: MAX_STORED_QUESTIONS_PER_JOB + 10 }, (_, i) => ({ id: `q${i}` }));
    act(() => {
  ...
  it('updateJob merges updates and truncates storedQuestions', async () => {
    ...
    const many = Array.from({ length: MAX_STORED_QUESTIONS_PER_JOB + 3 }, (_, i) => ({ id: `q${i}` }));
    act(() => {
