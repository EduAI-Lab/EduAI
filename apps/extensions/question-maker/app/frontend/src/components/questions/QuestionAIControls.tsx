...
export function QuestionAIControls({
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-secondary to-accent"
      />
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div className="flex flex-col">
            ...
            <p className="text-xs text-muted-foreground">
              {mode === "variant"
                ? "Spin a variant from a prompt"
                : "Draft a question from a prompt"}
            </p>
          </div>
        </div>
        <AIServiceIndicators
      ...
      {status === "error" && isUbcModel && (
        <div className="rounded-md border border-red-200 bg-red-50 dark:bg-red-950/30 dark:border-red-900 px-3 py-2 text-xs text-red-700 dark:text-red-300">
          <span className="font-semibold">UBC-hosted model unavailable.</span> Connect to UBC
          wifi/VPN, or pick a cloud model above and add its API key.
        </div>
      )}
        ...
        <div className="space-y-1.5">
          ...
          {keySaved ? (
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Button type="button" size="sm" variant="outline" onClick={onClearSavedKey}>
                Change
              </Button>
            </div>
          ) : (
          ...
          <p className="text-[11px] text-muted-foreground">
            Stored for this account in this browser and sent through EduAI services when you use AI.
