...
export const AddQuestionDialog = (props: AddQuestionDialogProps) => {
  ...
  useEffect(() => {
    ...
    courseService
      ...
      .then((list) => {
        if (!cancelled) setViewTopics(list);
      })
      .finally(() => {
        if (!cancelled) setViewTopicsLoading(false);
      });
    return () => {
      cancelled = true;
    };
  ...
  useEffect(() => {
    ...
    isViewMode,
    editingMetadata,
    viewEntry?.questionDescription,
    viewEntry?.primaryTopicId,
    viewEntry?.questionType,
    viewVariant?.difficulty,
    viewVariant?.questionText,
    viewVariant?.secondaryTopicsId,
  ]);
  ...
  const handleToggleAiTag = async () => {
    ...
    try {
      ...
      toast.error("Failed to toggle AI tag", {
  ...
  const isExternalGenerationModel = useMemo(
    () =>
      selectedGenerationModel
        ? selectedGenerationModel.provider !== "ollama"
        : !form.generationModel.startsWith("ollama"),
  ...
  useEffect(() => {
    ...
    if (presetVariant) {
      ...
      if (
        presetVariant.questionType === "MCQ" &&
        presetVariant.variant.choices &&
        Array.isArray(presetVariant.variant.choices) &&
        presetVariant.variant.choices.length > 0
      ) {
      ...
      setForm({
        ...
        variantAssessmentId: presetVariant.variant.assessmentId
          ? presetVariant.variant.assessmentId.toString()
          : "none",
        generationPrompt: "",
      });
    } else {
  ...
  const assessmentOptions = useMemo(
    () =>
      assessments.filter((assessment) =>
        createProps?.courseId
          ? assessment.courseId === undefined || assessment.courseId === createProps.courseId
          : true,
      ),
  ...
  const handleFieldChange = <K extends keyof FormState>(field: K, value: FormState[K]) => {
    if (field === "primaryTopicId" && isString(value)) {
      setForm((prev) => ({
        ...
        variantSecondaryTopics: prev.variantSecondaryTopics.filter(
          (topicId) => topicId.toString() !== value,
        ),
      }));
      return;
    }
    ...
    if (!code) {
      const fallback = availableEduCourses.find((eduCourse) => {
        ...
        if (createProps?.courseId) {
          ...
          if (matchesId)
            console.log("[AI service] fallback matched by id", {
              eduCourse,
              courseId: createProps.courseId,
            });
          return matchesId;
        }
        return false;
  ...
  const requiredValidation = useMemo(() => {
    ...
    if (!form.variantText.trim())
      missing.push({ label: "Question text", fieldId: "field-variant-text" });
    if (createMode === "variant") {
      if (!form.baseSelection) missing.push({ label: "Base variant selection", fieldId: null });
    } else {
      if (!form.primaryTopicId.trim())
        missing.push({ label: "Primary topic", fieldId: "field-primary-topic" });
    }
  ...
  const handleCreateClick = () => {
    if (requiredValidation.missingLabels.length > 0 && requiredValidation.firstFieldId) {
      document
        .querySelector(`[data-field-id="${requiredValidation.firstFieldId}"]`)
  ...
  const handleGenerateWithAI = async () => {
    ...
    try {
      ...
      processingToast = toast(
        createMode === "variant"
          ? "Variant generation in progress"
      ...
      setForm((prev) => {
        ...
        if (createMode === "variant") {
          return {
            ...
            variantSecondaryTopics:
              resolvedSecondaryTopics.length > 0
                ? resolvedSecondaryTopics
    ...
    try {
      ...
      if (createMode === "variant") {
        ...
        await questionService.createVariant(questionId, {
          ...
          assessmentId:
            form.variantAssessmentId === "none" ? undefined : parseNumber(form.variantAssessmentId),
          secondaryTopicsId: form.variantSecondaryTopics.length
            ? form.variantSecondaryTopics
            : undefined,
      ...
      const description =
        form.questionDescription.trim() || createDescriptionFromText(form.variantText) || null;
            ...
            {siblingVariants.length > 1 && (
              <aside className="shrink-0 overflow-y-auto border-b border-border bg-muted/30 p-3 md:w-60 md:border-b-0 md:border-r">
                ...
                <div className="flex gap-2 overflow-x-auto pb-1 md:flex-col md:overflow-visible md:pb-0">
                  {(() => {
                    let n = 0;
                    return siblingVariants.map((item) => ({
                      item,
                      isOriginal: item.variant.referenceId == null,
                      railLabel: item.variant.referenceId == null ? "Original" : `Variant ${++n}`,
                    }));
                  })().map(({ item, isOriginal, railLabel }) => {
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <span
                            className={cn(
                              ...
                              isOriginal ? "text-primary-text" : "text-foreground",
                            )}
                          >
              ...
              <div className="mx-auto flex max-w-3xl flex-col gap-6 p-6">
                ...
                <section>
                  ...
                  <div className="border-l-2 border-accent pl-4">
                    <div className={cn("font-semibold text-foreground", questionTextClass)}>
                      {questionTextDisplay}
                    </div>
                  </div>
                </section>
                    ...
                    <section>
                      ...
                      <div className="mt-3 flex gap-2">
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const validChoices = editChoices.filter(
                              ...
                              const saved = await questionService.updateVariant(
                                viewVariant.id,
                                payload,
                              );
                        ...
                        {canEditDraft && (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setEditAnswer(
                                viewVariant.answer?.trim().toUpperCase().charAt(0) ?? "",
                              );
                        ...
                        <p className="text-sm font-medium leading-relaxed text-foreground whitespace-pre-line">
                          ...
                          viewVariant.choices.length > 0
                            ? (() => {
                                ...
                                const letter = (viewVariant.answer ?? "")
                                  .trim()
                                  .toUpperCase()
                                  .charAt(0);
                ...
                <section className="rounded-[var(--radius-lg)] border border-border bg-card p-5">
                  ...
                  {editingMetadata ? (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                        <div className="space-y-2">
                          ...
                          <Select
                            value={editType}
                            onValueChange={(v) => setEditType(v as QuestionType)}
                            disabled={isApproved}
                          >
                            <SelectTrigger>
                              <SelectValue />
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              const primaryTopicChanged =
                                editPrimaryTopicId !==
                                (viewEntry.primaryTopicId != null
                                  ? String(viewEntry.primaryTopicId)
                                  : "");
                              ...
                              const variantUpdates = buildVariantMetadataUpdates({
                                isDraft: !isApproved,
                                currentQuestionText: viewVariant.questionText ?? "",
                                editQuestionText,
                                currentDifficulty:
                                  (viewVariant.difficulty as QuestionDifficulty) ?? "medium",
                                editDifficulty,
                              });
                              ...
                              const currentSecondary = (viewVariant.secondaryTopicsId ?? [])
                                .map(String)
                                .sort();
                              ...
                              const primaryTopicName = viewTopics.find(
                                (t) => t.id === editPrimaryTopicId,
                              )?.name;
                              // onUpdateQuestionMetadata refetches the question and rebuilds secondaryTopicNames, so saved tags refresh in-place.
                              vp.onUpdateQuestionMetadata?.(viewEntry.questionId, {
                              ...
                              if (Object.keys(variantUpdates).length > 0)
                                vp.onUpdateVariant?.(viewEntry.variant.id, variantUpdates);
                              setEditingMetadata(false);
                              toast("Question details saved", {
                                description:
                                  Object.keys(variantPayload).length > 0
                                    ? "Question details and variant fields updated."
                                    : "Question details updated.",
                              });
                            } catch (err: unknown) {
                              toast.error("Failed to save", {
                                description:
                                  err instanceof Error ? err.message : "Could not update metadata.",
                              });
                            } finally {
                    ...
                    <dl className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
                      <Fact
                        label="Description"
                        value={viewEntry.questionDescription || "—"}
                        spanFull
                      />
                      ...
                      <Fact
                        label="In assessment"
                        value={
                          viewVariant.assessment ? (
                            <button
                              type="button"
                              onClick={() =>
                                navigate(
                                  `/courses/${viewEntry.courseId}/assessments/${viewVariant.assessment!.id}`,
                                )
                              }
                    ...
                    <section className="rounded-[var(--radius-lg)] border border-border bg-muted/40 p-5 space-y-3">
                      ...
                      {!coreQuestionId ? (
                        ...
                        <div className="flex items-center gap-3">
                          <Switch
                            id={`testable-${viewEntry.variant.id}`}
                            checked={isTestable}
            ...
            <div className="flex items-center gap-2">
              <PermissionGate allow={canCreateQuestion}>
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => vp.onCreateVariant(viewEntry)}
              ...
              <PermissionGate allow={canDeleteResource(viewOwner)}>
                <Button
                  type="button"
                  variant="destructive"
                  onClick={() => vp.onDeleteVariant(viewEntry)}
                  className="flex items-center gap-2"
                >
        ...
        <div className="h-[70vh] min-h-0 overflow-y-auto pr-1">
          <div className="flex flex-col gap-4 pb-4">
            {form.variantReferenceId && (
              <p className="text-xs text-muted-foreground">
                Reference variant ID: <span className="font-mono">{form.variantReferenceId}</span>{" "}
            ...
            <div className="rounded-lg border border-border bg-card p-5" data-tour-id="aq-metadata">
              ...
              <QuestionMetadataPanel
                value={{
                  ...
                  variantAssessmentId: form.variantAssessmentId,
                }}
                onChange={(field, value) => handleFieldChange(field, value)}
                topics={topics}
                ...
                mode={createMode}
                primaryTopicName={
                  createMode === "variant" && presetVariant
            ...
            {createButtonTooltip ? (
              <Tooltip content={createButtonTooltip} side="top" multiline>
                <span className="inline-block">
                  <Button
                    type="button"
                    variant={createMode === "new" && !markAsReviewed ? "outline" : "default"}
                    onClick={handleCreateClick}
                    disabled={isSubmitting}
                    className={isCreateDisabled ? "cursor-pointer" : ""}
                  >
                    {isSubmitting
                      ? "Saving..."
                      : createMode === "new"
