    ...
    >
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          We support PDF, PNG, JPG, TXT and other common formats.
        </p>
      </div>
      <input
        id={id}
...
export function mapExtractedToDraftQuestions(items: ExtractedQuestion[]): DraftQuestion[] {
  return items
    .filter((item): item is ExtractedQuestion & { summary: string } =>
      Boolean(item.question?.trim() && item.summary?.trim()),
    )
    .map((item) => {
      const primaryTopicId =
        item.primaryTopicId !== undefined &&
        item.primaryTopicId !== null &&
      ...
      const normalizedChoices: MCQChoice[] | null =
        isMCQ && Array.isArray(item.choices) && item.choices.length >= 2
          ...
          : null;
      const choices: MCQChoice[] | null = isMCQ
      ...
      return {
        ...
        difficulty:
          item.difficulty && ["easy", "medium", "hard"].includes(item.difficulty)
            ? (item.difficulty as QuestionDifficulty)
            : "medium",
        answer: item.answer ?? "",
        type:
          item.type && ["MCQ", "SA", "LA"].includes(item.type) ? (item.type as QuestionType) : "SA",
        ...
        correctAnswers: Array.isArray(item.correctAnswers)
          ? item.correctAnswers.map((l) => String(l))
          : [],
...
export const QuestionUploadDialog = ({
  ...
  const [processingStage, setProcessingStage] = useState<
    "idle" | "ocr" | "extracting" | "review" | "saving"
  >("idle");
  const [progress, setProgress] = useState<number>(0);
  const [error, setError] = useState<string | null>(null);
  const [lastFileName, setLastFileName] = useState<string>("");
  const [assessmentType, setAssessmentType] =
    useState<(typeof assessmentTypes)[number]>("Assignment");
  ...
  const { jobs: ocrJobs, addJob, updateJobStatus, removeJob, clearHistory } = useOCRHistory();
  ...
  const isExternalModel = useMemo(
    () => (selectedModel ? !isCampusModel(selectedModel) : apiKeyStorage.requiresApiKey(aiModel)),
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const message =
          err?.response?.data?.error || err?.message || "Failed to extract questions.";
        setError(message);
        setProcessingStage("idle");
        setProgress(0);
  ...
  useEffect(() => {
    if (currentJobId && draftQuestions.length > 0 && processingStage === "review") {
      const storedQuestions: StoredQuestion[] = draftQuestions.map((draft) => ({
        ...
        ...(draft.type === "MCQ" &&
          draft.choices &&
          draft.choices.length >= 2 && { choices: draft.choices }),
        ...(draft.answer != null && draft.answer !== "" && { answer: draft.answer }),
        ...(draft.type === "MCQ" &&
          draft.selectAllThatApply && {
  ...
  const performRestoreFromJob = useCallback(
    (job: OCRJob) => {
      ...
      const restoredDrafts: DraftQuestion[] = job.storedQuestions.map((sq) => ({
        ...
        choices:
          sq.type === "mcq"
            ? sq.choices && sq.choices.length >= 2
              ? (sq.choices as MCQChoice[])
              : defaultMcqChoices
            : null,
        instructions: "",
        ...
        correctAnswers:
          sq.type === "mcq" && Array.isArray(sq.correctAnswers)
            ? sq.correctAnswers.map(String)
            : [],
      }));
    ...
    const payloadQuestions = includedDrafts.map((draft) => ({
      ...
      ...(draft.type === "MCQ" &&
        ...
        draft.choices.length >= 2 && {
          ...
          correctAnswers:
            draft.selectAllThatApply && Array.isArray(draft.correctAnswers)
  ...
  return (
    <>
      <Dialog
        open={open}
        onOpenChange={(value) => {
          if (!value) handleCloseAttempt();
        }}
      >
        <DialogContent
          className={`max-h-[90vh] overflow-hidden flex flex-col gap-0 p-0 transition-all duration-200 ${showHistoryPanel ? "max-w-[95vw] sm:max-w-[1400px]" : "max-w-6xl sm:max-w-7xl w-[95vw]"}`}
        >
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <DialogDescription>
                  ...
                  <span className="font-medium text-foreground">
                    {courseName ?? "the selected course"}
                  </span>
                  . We&apos;ll extract them with OCR (or use text as-is for TXT) and AI so you can
              ...
              <Tooltip
                content={
                  showHistoryPanel
                    ? "Hide upload history"
                    : "View past uploads. Unsaved questions are kept in History when you close."
                }
                side="left"
              >
                <Button
                ...
                >
                  ...
                  History
                  {ocrJobs.filter((j) => j.status === "processing" || j.status === "pending")
                    .length > 0 && (
                      ...
                      {!uploadSectionCollapsed && (
                        <Card className="mt-2">
                          <CardHeader className="space-y-1">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Questions will be saved to{" "}
                              <span className="font-medium text-foreground">
                                {courseName ?? "the selected course"}
                              </span>
                              . Topics are assigned automatically after extraction—you can adjust
                            ...
                            <div className="space-y-2" data-tour-id="upload-model">
                              ...
                              {isExternalModel && (
                                <div className="w-full rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                                  <span className="font-semibold">Warning:</span> External models
                                  send your prompts and course data to that provider. UBC-hosted
                                  models keep data within UBC systems.
                                </div>
                              )}
                            </div>
                                ...
                                {providerApiKey ? (
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <Button
                                      ...
                                      size="sm"
                                      disabled={
                                        !providerApiKey.trim() || apiKeySaveState === "saving"
                                      }
                              ...
                              <div key={draft.id} className="space-y-4 p-4">
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  <div className="flex items-center gap-2">
                                    <Button
                                      ...
                                      size="sm"
                                      onClick={() =>
                                        updateDraft(draft.id, { include: !draft.include })
                                      }
                                    >
                                ...
                                <div className="space-y-2">
                                  ...
                                  <Input
                                    ...
                                    onChange={(event) =>
                                      updateDraft(draft.id, { summary: event.target.value })
                                    }
                                  />
                                </div>
                                <div className="space-y-2">
                  ...
                  <Card>
                    <CardHeader className="space-y-1">
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        . Topics are assigned automatically after extraction—you can adjust them in
                        the review step.
                      </p>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="space-y-2" data-tour-id="upload-model">
                        ...
                        {isExternalModel && (
                          <div className="w-full rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
                            <span className="font-semibold">Warning:</span> External models send
                            your prompts and course data to that provider. UBC-hosted models keep
                            data within UBC systems.
                          </div>
                        )}
                      </div>
          ...
          <DialogFooter className="shrink-0 border-t border-border px-6 py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="text-sm text-muted-foreground">
              {includedDrafts.length} question{includedDrafts.length === 1 ? "" : "s"} ready to
              save.
            </div>
            <div className="flex gap-2 sm:justify-end">
              <Tooltip
                content={
                  draftQuestions.length > 0 && processingStage === "review"
                    ? "Close will ask you to save or discard your current questions."
                    : "Close this dialog."
                }
                side="top"
              >
      ...
      <AlertDialog
        open={!!pendingRestoreJob}
        onOpenChange={(open) => {
          if (!open) setPendingRestoreJob(null);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              You have unsaved questions in this session. Restoring from history will replace them.
              Your current set will remain in History as discarded so you can switch back later if
              needed.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => {
          if (!open) setShowClearHistoryConfirm(false);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This removes all jobs from the History list. Questions you already saved are
              unaffected. You will no longer be able to restore past uploads from this list.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setShowClearHistoryConfirm(false)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
              onClick={() => {
