...
export const refreshEduAIStatus = async (opts?: {
  silent?: boolean;
  ...
  if (!opts?.silent) {
    setState({
      status: "loading",
      message: "Checking AI service status. Connect to UBC wifi or VPN.",
    });
  }
...
export const useEduAIStatus = () => {
  ...
  const { status, message, provider, questionGenerationPhase } = useSyncExternalStore(
    subscribe,
    getSnapshot,
    getSnapshot,
  );
