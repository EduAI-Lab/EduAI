...
describeDb(
  "assessmentService — deleteAssessment and getQuestionsInAssessment (integration)",
  () => {
    ...
    async function makeQuestion(order = {}) {
      return prisma.questionMetadata.create({
        data: { courseId, primaryTopicId: topicId, type: "SA", questionOrder: order },
      });
    }
    ...
    describe("getQuestionsInAssessment", () => {
      ...
      it("throws when the assessment does not belong to the requesting user", async () => {
        const assessment = await makeAssessment();
        await expect(getQuestionsInAssessment(assessment.id, "cuid-wrong-user")).rejects.toThrow(
          /assessment not found/i,
        );
      });
    });
