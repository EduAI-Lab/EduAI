/**
 * PICT model (#1188, census docs/PICT_CENSUS.md § S9): `loadBankMetadataWithVariants`
 * + `selectBestBankMetadata` + `scoreMetadataMatch` (../../src/services/assessmentVariantService.js,
 ...
 * `loadBankMetadataWithVariants` reads the bank from the DB; `selectBestBankMetadata`
 * is a pure scoring pass, and the baseline `slotVariant` it scores against is
  ...
  describe.each(rows.map((row, index) => ({ row, index })))(
    ...
    ({ row }) => {
      it('matches the oracle', async () => {
        ...
        const bankRows = await loadBankMetadataWithVariants({ courseId, client: prisma });
        const result = selectBestBankMetadata(slotVariant, bankRows, usedBankMetadataIds);
