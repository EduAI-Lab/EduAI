...
describeDb("cross-course write scoping (integration, #1)", () => {
  ...
  beforeAll(async () => {
    ...
    ({ createQuestion, updateQuestion, updateQuestionOrder, createVariant, updateVariant } =
  ...
  beforeEach(async () => {
    ...
    topicASecondary = await prisma.topics.findFirst({
      where: { courseId: courseA.id, id: { not: topicA.id } },
    });
  ...
  describe("legacy SectionVariants mutation guards", () => {
    it("rejects removing a malformed foreign-course link", async () => {
      ...
      expect(
        await prisma.sectionVariants.count({
          where: { sectionId: sectionA.id, variantId: variantB.id },
        }),
      ).toBe(1);
    });
    ...
    it("does not remove a question link from a foreign-course section", async () => {
      ...
      const result = await sectionSvc.removeQuestionFromAllSections(
        questionA.id,
        USER.id,
        courseA.id,
      );
      ...
      expect(
        await prisma.sectionVariants.count({
          where: { sectionId: sectionB.id, variantId: variantA.id },
        }),
      ).toBe(1);
    });
  });
  ...
  describe("resource relocation and relation integrity", () => {
    ...
    it.each([
      [
        "assessment",
        ({ assessmentB: foreignAssessment }) => ({ assessmentId: foreignAssessment.id }),
        /Assessment not found for this course/,
      ],
      [
        "secondary topic",
        ({ topicB: foreignTopic }) => ({ secondaryTopicsId: [foreignTopic.id] }),
        /Secondary topic not found for this course/,
      ],
      [
        "reference variant",
        ({ variantB: foreignVariant }) => ({ referenceId: foreignVariant.id }),
        /Reference variant not found for this course/,
      ],
    ])(
      "rejects creating a variant with a foreign-course %s",
      async (_label, payloadFactory, matcher) => {
    ...
    it.each([
      ...
      "rejects updating a variant with a foreign-course %s",
      async (_label, payloadFactory, matcher) => {
        const payload = payloadFactory({ assessmentB, topicB, variantB });
        await expect(updateVariant(variantA.id, payload, USER.id)).rejects.toThrow(matcher);
      },
    );
