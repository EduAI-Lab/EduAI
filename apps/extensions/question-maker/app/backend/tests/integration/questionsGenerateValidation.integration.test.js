...
beforeEach(() => {
  ...
  mockCourseFindOne.mockResolvedValue({ id: 1, userId: TEST_USER.id, coreCourseId: 'cuid-core-course' });
  ...
  describe("POST /api/questions/generate", () => {
    ...
    it('rejects an oversized prompt before invoking a provider', async () => {
      const res = await request(app)
        .post('/api/questions/generate')
        .set('Cookie', 'session=valid')
        .send({ courseId: 1, prompt: 'x'.repeat(config.qmGeneratePromptMaxChars + 1), numQuestions: 1 });
    ...
    it('rejects a course the caller cannot author in before invoking a provider', async () => {
      mockCourseFindOne.mockResolvedValue({ id: 1, userId: 'other-owner', coreCourseId: 'cuid-core-course' });
