...
describeDb("QM cross-course authorization (real DB)", () => {
  ...
  function stubCore({ enrolledCourses = [] } = {}) {
    const fetchMock = vi.fn(async (input) => {
      ...
      if (scopedCoursesMatch) {
        ...
        return coreResponse({
          data: ids.filter((id) => enrolledCourses.includes(id)).map((id) => ({ id })),
  ...
  beforeEach(async () => {
    ...
    courseA = await prisma.course.create({
      data: { userId: OWNER.id, coreCourseId: "qm-xc-core-a" },
    });
    courseB = await prisma.course.create({
      data: { userId: OWNER.id, coreCourseId: "qm-xc-core-b" },
    });
    topicA = await prisma.topics.create({
      data: { id: "qm-xc-topic-a", name: "A", courseId: courseA.id },
    });
    ...
    assessmentA = await prisma.assessments.create({
      data: { courseId: courseA.id, type: "Quiz", name: "A" },
    });
  });
  ...
  it("keeps a linked anchor and all content on A when a caller can see target B", async () => {
    ...
    expect(
      fetchMock.mock.calls.some(([url]) => String(url).includes(`ids=${courseB.coreCourseId}`)),
    ).toBe(false);
