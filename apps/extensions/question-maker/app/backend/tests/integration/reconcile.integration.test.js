...
describeDb("runReconciliation (integration)", () => {
  ...
  it("deletes the course (and cascades) when Core returns 404 — §802 reconcile safety net", async () => {
    const course = await prisma.course.create({
      data: { userId: TEST_USER.id, coreCourseId: "core-cuid-1" },
    });
    const topic = await prisma.topics.create({
      data: { id: createId(), courseId: course.id, name: "Chapter 1" },
    });
    await prisma.questionMetadata.create({
      data: { courseId: course.id, primaryTopicId: topic.id, type: "SA" },
    });
    ...
    await expect(prisma.questionMetadata.count({ where: { courseId: course.id } })).resolves.toBe(
      0,
    );
  });
  ...
  it("leaves columns intact when Core returns 5xx", async () => {
    const course = await prisma.course.create({
      data: { userId: TEST_USER.id, coreCourseId: "core-cuid-2" },
    });
