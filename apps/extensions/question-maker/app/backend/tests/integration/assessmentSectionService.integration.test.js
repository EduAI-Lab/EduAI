...
describeDb("assessmentSectionService (integration)", () => {
  ...
  beforeEach(async () => {
    ...
    await prisma.user.create({
      data: { id: STRANGER.id, email: STRANGER.email, name: STRANGER.name },
    });
  ...
  describe("createAssessmentSection", () => {
    ...
    it("honors an explicit position and trims/defaults fields", async () => {
      const s = await svc.createAssessmentSection(assessmentId, USER.id, {
        name: "   ",
        position: 5,
      });
    ...
    it("throws when the assessment does not belong to the user", async () => {
      await expect(
        svc.createAssessmentSection(assessmentId, STRANGER.id, { name: "X" }),
  ...
  describe("updateAssessmentSection", () => {
    it("updates provided fields and verifies ownership", async () => {
      ...
      const updated = await svc.updateAssessmentSection(s.id, USER.id, {
        name: "New",
        position: 3,
        description: "desc",
      });
    ...
    it("throws when the section does not exist", async () => {
      await expect(svc.updateAssessmentSection(999999, USER.id, { name: "Y" })).rejects.toThrow(
        /Section not found/,
      );
    });
  });
  ...
  describe("variant linking", () => {
    ...
    it("throws when linking a variant the user does not own", async () => {
      const s = await svc.createAssessmentSection(assessmentId, USER.id, { name: "S" });
      await expect(svc.addVariantToSection(s.id, USER.id, 999999)).rejects.toThrow(
        /Variant not found/,
      );
    });
    ...
    it("throws when removing a variant absent from the section", async () => {
      const s = await svc.createAssessmentSection(assessmentId, USER.id, { name: "S" });
      const v = await makeVariant();
      await expect(svc.removeVariantFromSection(s.id, USER.id, v.id)).rejects.toThrow(
    ...
    it("throws when reordering a variant absent from the section", async () => {
      const s = await svc.createAssessmentSection(assessmentId, USER.id, { name: "S" });
      const v = await makeVariant();
      await expect(svc.updateVariantOrderInSection(s.id, USER.id, v.id, 1)).rejects.toThrow(
        /not found in section/,
      );
    });
  });
  ...
  describe("checkQuestionInAssessments / removeQuestionFromAllSections", () => {
    ...
    it("reports the assessments a linked question belongs to, then removes all links", async () => {
      ...
      const v = await prisma.variants.create({
        data: { questionMetadataId: qm.id, questionText: "Linked?", difficulty: "easy" },
      });
      await svc.addVariantToSection(s.id, USER.id, v.id);
    ...
    it("throws when the question is not owned by the user", async () => {
      ...
      await expect(svc.checkQuestionInAssessments(qm.id, STRANGER.id)).rejects.toThrow(
        /Question not found/,
      );
      await expect(svc.removeQuestionFromAllSections(qm.id, STRANGER.id)).rejects.toThrow(
        /Question not found/,
      );
    });
  });
  ...
  describe("reorderAssessmentSections", () => {
    it("rewrites positions to 0..n-1 in the given order", async () => {
      ...
      const reordered = await svc.reorderAssessmentSections(assessmentId, USER.id, [
        c.id,
        a.id,
        b.id,
      ]);
    ...
    it("rejects incomplete, duplicate, or foreign section ids", async () => {
      ...
      await expect(svc.reorderAssessmentSections(assessmentId, USER.id, [a.id])).rejects.toThrow(
        /sectionIds/i,
      );
      await expect(
        svc.reorderAssessmentSections(assessmentId, USER.id, [a.id, a.id]),
      ).rejects.toThrow(/sectionIds/i);
      await expect(
        svc.reorderAssessmentSections(assessmentId, USER.id, [a.id, b.id, 999999]),
      ).rejects.toThrow(/sectionIds/i);
    });
    ...
    it("throws when the assessment does not belong to the user", async () => {
      const a = await svc.createAssessmentSection(assessmentId, USER.id, { name: "A" });
      await expect(
        svc.reorderAssessmentSections(assessmentId, STRANGER.id, [a.id]),
      ).rejects.toThrow(/Assessment not found/);
    });
  });
});
