...
const { createQuestion, updateQuestion, createMultipleQuestions, updateQuestionOrder } =
...
describe("question order service guards", () => {
  ...
  it.each([0, -1, 1.5, Number.MAX_SAFE_INTEGER + 1, Infinity, NaN])(
    ...
    async (orderNumber) => {
      await expect(updateQuestionOrder(101, 10, orderNumber, "user-1")).rejects.toThrow(
        /positive safe integer/i,
      );
      await expect(addQuestionToAssessment(10, 101, orderNumber, "user-1")).rejects.toThrow(
        /positive safe integer/i,
      );
    },
  );
