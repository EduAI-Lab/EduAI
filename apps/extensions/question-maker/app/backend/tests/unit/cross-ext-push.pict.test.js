...
const { shouldPushApprovedVariantToCore } = await import("../../src/services/variant-push-gate.js");
...
async function runPush(row) {
  ...
  if (row.Session === "missing") {
    ...
    vi.mocked(fetch).mockResolvedValue(jsonResponse(409, { error: "IDEMPOTENCY_IN_PROGRESS" }));
