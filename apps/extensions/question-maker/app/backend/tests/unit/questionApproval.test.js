...
import { parseApprovalTarget, prepareApprovalQuestions } from "../../src/utils/questionApproval.js";
...
describe("prepareApprovalQuestions", () => {
  ...
  it.each([
    ...
    expect(
      prepareApprovalQuestions([{ ...override, primaryTopicId: "topic-1" }], {
        targetCourseId: 12,
        createdBy: "caller-1",
  ...
  it("rejects an invalid primary topic after course overrides are validated", () => {
    expect(
      prepareApprovalQuestions([{ courseId: 12, primaryTopicId: "   " }], {
        targetCourseId: 12,
        createdBy: "caller-1",
        normalizeTopicId,
      }),
  ...
  it("rejects a question whose courseId and classId conflict", () => {
    ...
    expect(
      prepareApprovalQuestions([{ courseId: 12, classId: 13, primaryTopicId: "topic-1" }], {
        targetCourseId: 12,
        createdBy: "caller-1",
        normalizeTopicId: normalizeTopicIdSpy,
      }),
  ...
  it("accepts a question whose courseId and classId match the target", () => {
    expect(
      prepareApprovalQuestions([{ courseId: "12", classId: 12, primaryTopicId: "topic-1" }], {
        targetCourseId: 12,
        createdBy: "caller-1",
        normalizeTopicId,
      }),
