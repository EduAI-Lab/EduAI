...
const envCandidates = [join(__dirname, "../../../.env"), join("/app", ".env"), join("/", ".env")];
...
const run = async () => {
  try {
    ...
    console.log(
      `Found ${courses.length} course(s). Adding dev-seed topics and questions to each...\n`,
    );
    ...
    for (let courseIndex = 0; courseIndex < courses.length; courseIndex++) {
      ...
      console.log(
        `  Course: ${courseLabel} (template ${templateIndex}: ${topicNames.length} topics, ${questions.length} questions)`,
      );
