...
export const qmGeneratePromptMaxChars = () => configInt("qmGeneratePromptMaxChars", 12_000);

export const qmMaxQuestions = () => configInt("maxQuestions", 50);
export const qmAiRateLimitWindowMs = () => configInt("qmAiRateLimitWindowMs", 15 * 60 * 1000);
...
export function validateBankVariantAdmission(body = {}) {
  ...
  if (!questionIds || questionIds.length === 0) {
    return publicValidation(
      400,
      "QM_BANK_QUESTION_IDS_REQUIRED",
      "questionIds is required and must be a non-empty array",
    );
  }
  ...
  for (const rawId of questionIds) {
    ...
    if (id == null) {
      return publicValidation(
        400,
        "QM_BANK_QUESTION_ID_INVALID",
        "questionIds must contain finite positive integer ids",
      );
    }
    if (seen.has(id)) {
      return publicValidation(
        400,
        "QM_BANK_QUESTION_IDS_DUPLICATE",
        "questionIds must not contain duplicate ids",
      );
    }
    seen.add(id);
  ...
  if (variantsToAdd == null) {
    return publicValidation(
      400,
      "QM_BANK_VARIANTS_INVALID",
      "variantsToAdd must be a positive integer",
    );
  }
...
export function validateReviewAdmission(body = {}) {
  ...
  if (body.includeOverallSummary != null && typeof body.includeOverallSummary !== "boolean") {
    return publicValidation(
      400,
      "QM_REVIEW_SUMMARY_FLAG_INVALID",
      "includeOverallSummary must be boolean",
    );
  }
  return {
    ...
    providerCalls:
      body.includeOverallSummary === false ? 2 * qmReviewMaxPairs() : qmReviewMaxProviderCalls(),
  };
}
...
export function validateChatAdmission(body = {}) {
  ...
  if (!messages)
    return publicValidation(400, "QM_CHAT_MESSAGES_REQUIRED", "Messages array is required");
  const maxMessages = qmChatMaxMessages();
  if (messages.length > maxMessages) {
    return publicValidation(
      400,
      "QM_CHAT_MESSAGE_COUNT_TOO_LARGE",
      `messages cannot contain more than ${maxMessages} entries`,
    );
  }
  ...
  for (const message of messages) {
    ...
    if (typeof message.content !== "string") {
      return publicValidation(
        400,
        "QM_CHAT_MESSAGE_CONTENT_INVALID",
    ...
    if (message.content.length > maxMessageChars) {
      return publicValidation(
        400,
        "QM_CHAT_MESSAGE_TOO_LARGE",
        `Each message cannot exceed ${maxMessageChars.toLocaleString()} characters`,
      );
    }
    ...
    if (aggregateChars > maxAggregateChars) {
      return publicValidation(
        400,
        "QM_CHAT_AGGREGATE_TOO_LARGE",
        `Chat content cannot exceed ${maxAggregateChars.toLocaleString()} characters`,
      );
    }
  }
...
export function validateTestApiKeyAdmission(body = {}) {
  ...
  try {
    ...
    return publicValidation(
      400,
      "QM_TEST_API_KEY_BODY_INVALID",
      "Request body must be finite JSON",
    );
  }
  ...
  if (Buffer.byteLength(serialized, "utf8") > qmTestApiKeyMaxBodyBytes()) {
    return publicValidation(
      413,
      "QM_TEST_API_KEY_BODY_TOO_LARGE",
      `Request body cannot exceed ${qmTestApiKeyMaxBodyBytes()} bytes`,
    );
  }
  ...
  for (const [provider, rawValue] of Object.entries(supplied)) {
    ...
    if (!TEST_PROVIDERS.has(normalizedProvider)) {
      return publicValidation(
        400,
        "QM_TEST_API_KEY_PROVIDER_INVALID",
        "apiKeys contains an unsupported provider",
      );
    }
    ...
    if (key.length > qmTestApiKeyMaxProviderKeyChars()) {
      return publicValidation(
        413,
        "QM_TEST_API_KEY_TOO_LARGE",
        `Provider keys cannot exceed ${qmTestApiKeyMaxProviderKeyChars()} characters`,
      );
    }
  ...
  if (
    active.length > 1 ||
    (forceProvider && active.length === 1 && active[0].provider !== forceProvider)
  ) {
    return publicValidation(
      400,
      "QM_TEST_API_KEY_AMBIGUOUS_PROVIDER",
      "Select exactly one provider for each probe",
    );
  }
  ...
  return {
    provider: forceProvider || active[0]?.provider || null,
    apiKeys: supplied,
    providerCalls: 1,
  };
}
...
export function setQmAiProviderReservationLimitForTests(n) {
  maxProviderReservationKeys =
    Number.isInteger(n) && n > 0 ? n : DEFAULT_MAX_PROVIDER_RESERVATION_KEYS;
}
...
function sendAdmissionError(res, validation) {
  ...
  return res
    .status(status)
    .json({ success: false, error: validation.message, code: validation.code });
}

/** Reserve a route's worst-case provider-call cost and start its shared deadline. */
export function qmAiProviderCallAdmission({
  validate,
  getCost = (req) => req?.aiAdmission?.providerCalls ?? 1,
} = {}) {
  return (req, res, next) => {
