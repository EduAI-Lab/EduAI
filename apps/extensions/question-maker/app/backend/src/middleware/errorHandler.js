...
export const errorHandler = (err, req, res, next) => {
  ...
  if (err instanceof Prisma.PrismaClientKnownRequestError) {
    if (err.code === 'P2002') {
      ...
      error = { message: 'Referenced resource does not exist', status: 400, code: 'RESOURCE_REFERENCE_INVALID' };
  ...
  const safeCode = [
    ...
    /^(?:QM_|PAGINATION_|CORE_|COURSE_|EDUAI_|AUTH_|CSRF_|RESOURCE_|VARIANT_|QUESTION_|ASSESSMENT_|INVALID_|DUPLICATE_|BUG_|PROVIDER_)/.test(candidate)
  ));

  const status = Number.isInteger(error.status) ? error.status : 500;
  const isStableUpstream = err?.isSanitizedUpstreamError === true;
  const hasExplicitPublicStatus = err?.isPublic === true || err?.name === 'PaginationError' || Boolean(safeCode);
  const message = error.message || (
    (isStableUpstream || err?.isPublic === true || hasExplicitPublicStatus)
      ? err?.message
      : null
  );
