...
export function parseQuestionListFilters(query = {}) {
  const types = parseCsvParam(query.types ?? query.type).filter((t) => ALLOWED_TYPES.has(t));
  const difficulties = parseCsvParam(query.difficulties ?? query.difficulty).filter((d) =>
    ALLOWED_DIFFICULTIES.has(d),
  );
  const reasoningLevels = parseCsvParam(query.reasoningLevels ?? query.reasoningLevel).filter((r) =>
    ALLOWED_REASONING.has(r),
  );

  const aiRaw = String(query.aiGenerated ?? "all")
    .trim()
    .toLowerCase();
  const aiGenerated = aiRaw === "ai" || aiRaw === "not-ai" || aiRaw === "all" ? aiRaw : "all";

  const draftRaw = String(query.draftStatus ?? "all")
    .trim()
    .toLowerCase();
  const draftStatus =
    draftRaw === "draft" || draftRaw === "reviewed" || draftRaw === "all" ? draftRaw : "all";

  const sortRaw = String(query.sortBy ?? "newest")
    .trim()
    .toLowerCase();
