...
Rules: no markdown, no code fences, no text before { or after }. Use double quotes on all keys and on brief_reason and usability. usability must be exactly usable_as_is OR usable_with_edits OR unusable (underscores). All six numeric fields must be integers 1–5.`;
  ...
  for (const q of perQuestion) {
    ...
    const distinctnessFactor = Number.isFinite(q.distinctness)
      ? distinctnessToFactor[q.distinctness]
      : null;
    q.exam_variant_distinctness_factor =
      typeof distinctnessFactor === "number" ? distinctnessFactor : null;
    ...
    q.exam_variant_composite_score_0to100_usability_adjusted =
      normalize1to5To0to100(adjustedComposite1to5);
  }
  ...
  const distinctnessFactorValues = perQuestion
    ...
    .filter((v) => typeof v === "number" && Number.isFinite(v));
  const distinctnessFactorAvg = distinctnessFactorValues.length
    ? distinctnessFactorValues.reduce((a, b) => a + b, 0) / distinctnessFactorValues.length
    : 1.0;

  const usabilityFactorValues = perQuestion
    .map((q) => usabilityMultiplier[q.usability] ?? 1.0)
    .filter((v) => typeof v === "number" && Number.isFinite(v));
  const usabilityFactorAvg = usabilityFactorValues.length
    ? usabilityFactorValues.reduce((a, b) => a + b, 0) / usabilityFactorValues.length
    : 1.0;
      ...
      : Math.max(
          ...
          Math.min(
            ...
            examVariantScoreBase0to100 *
              ...
              (applyUsabilityPenalty ? usabilityFactorAvg : 1.0),
          ),
        );
  ...
  const scoreAfterDistinctness0to100 =
    examVariantScoreBase0to100 == null ? null : examVariantScoreBase0to100 * distinctnessFactorAvg;
  const scoreAfterDistinctnessUsability0to100 =
    scoreAfterDistinctness0to100 == null
      ? null
      : scoreAfterDistinctness0to100 * (applyUsabilityPenalty ? usabilityFactorAvg : 1.0);
  ...
  const distinctnessValues = perQuestion
    ...
    .filter((v) => typeof v === "number" && Number.isFinite(v));
  const distinctnessAverage1to5 = distinctnessValues.length
    ? distinctnessValues.reduce((a, b) => a + b, 0) / distinctnessValues.length
    : null;
  ...
  function fallbackOverallSummary() {
    ...
    const distinctnessWeakness =
      typeof distinctnessAverage1to5 === "number" &&
      Number.isFinite(distinctnessAverage1to5) &&
      distinctnessAverage1to5 <= 2.0
    ...
    const baseWeaknesses = weaknessBits.length
      ? weaknessBits
      : ["At least one rubric dimension averaged lower across slots."];
    ...
    return {
      ...
      strengths: strengthBits.length
        ? strengthBits
        : ["Rubric dimensions were generally consistent across slots."],
