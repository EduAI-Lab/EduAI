...
export function scoreMetadataMatch(slotMeta, slotVariant, bankMeta, bankVariant) {
  let s = 0;
  if (slotMeta?.primaryTopicId != null && bankMeta?.primaryTopicId === slotMeta.primaryTopicId)
    s += 100;
  ...
  if (slotVariant?.reasoningLevel && bankVariant?.reasoningLevel === slotVariant.reasoningLevel)
    s += 10;
  return s;
}
