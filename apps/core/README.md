...
Every course-scoped chat turn injects course metadata and a scope policy into the system prompt (**Layer A**). Before the main model runs, a **classifier LLM** (**Layer B**, `course-scope-classifier.ts`) receives the course definition, RAG excerpts, conversation context, and the latest message — and returns `inScope` / refuse. No hardcoded off-topic word lists. Greetings skip the classifier. Disable Layer B with `CHAT_SCOPE_ZERO_CHUNK_GATE=0`. See [`docs/rag-ai/CHAT_RAG_PIPELINE.md`](../../docs/rag-ai/CHAT_RAG_PIPELINE.md).
