...
TEACHERS=(
  "Dr. Ada Lovelace|instructor.cs@eduai.local"
  "Dr. Emmy Noether|instructor.math@eduai.local"
)

STUDENTS=(
  "Alex Patel|student1@eduai.local|10000001"
  "Brooke Kim|student2@eduai.local|10000002"
  "Cameron Lee|student3@eduai.local|10000003"
  "Devon Singh|student4@eduai.local|10000004"
  "Erin Walsh|student5@eduai.local|10000005"
...
COURSES=(
  "Computer Studies|COSC 101|instructor.cs@eduai.local|student1@eduai.local,student2@eduai.local,student3@eduai.local"
  "Computer Programming II|COSC 121|instructor.cs@eduai.local|student1@eduai.local,student2@eduai.local,student4@eduai.local"
  "Machine Architecture|COSC 211|instructor.cs@eduai.local|student2@eduai.local,student4@eduai.local"
  "Calculus III|MATH 200|instructor.math@eduai.local|student1@eduai.local,student3@eduai.local,student5@eduai.local"
)
...
get_or_create_user() {
  ...
  if [[ -n "$existing" ]]; then
    ...
    existing="$(create_user "$name" "$email" "$password" "$sis_user_id")"
    uid="$(printf '%s' "$existing" | jq -r '.id')"
    log "created user: ${email} (id=${uid})"
  fi

  if [[ -n "$sis_user_id" ]]; then
    local actual_sis_user_id
    actual_sis_user_id="$(printf '%s' "$existing" | jq -r '.sis_user_id // empty')"
    [[ "$actual_sis_user_id" == "$sis_user_id" ]] || die "${email} has SIS id '${actual_sis_user_id:-missing}', expected '${sis_user_id}'"
  fi
...
main() {
  ...
  if [[ "$DO_RESET_ONLY" != true ]]; then
    ...
    log "  Courses: ${#COURSES[@]}"
    log ""
    log "Next: log in as instructor.cs@eduai.local, create a personal access token, and test roster APIs."
  fi
}
