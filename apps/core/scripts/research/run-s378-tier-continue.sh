#!/usr/bin/env bash
# Continue Tier A after P1: wait for P3b dev batch, then summarize + status report.
#
# Usage (SSH to s378):
...
P3B_OUT="${RESEARCH_P3B_OUT:-/tmp/policy-runs-p3b-dev.jsonl}"
...
wait_for_p3b() {
  ...
  if [[ "$n" -lt "$DEV_PROMPTS" ]]; then
    echo "P3b incomplete ($n/$DEV_PROMPTS) — starting batch"
    export RESEARCH_POLICY=P3b
    export RESEARCH_RUN_SPLIT=dev
    export RESEARCH_POLICY_OUT="$P3B_OUT"
    export RESEARCH_RUN_LABEL=tier-a-p3b-dev
    export RESEARCH_RUN_SLEEP_MS="${RESEARCH_RUN_SLEEP_MS:-500}"
    node scripts/research/run-policy-comparison.mjs || echo "P3b run had errors (continuing)"
  fi
  n=$(wc -l <"$P3B_OUT" 2>/dev/null || echo 0)
  echo "P3b done: $n rows"
}
...
echo "=== summarize P3b ==="
RESEARCH_POLICY_OUT="$P3B_OUT" node scripts/research/summarize-policy-runs.mjs | tee /tmp/policy-runs-p3b-dev-summary.txt
