...
import { isProviderConfigured, mergeServerOpenRouterApiKey } from "~/lib/ai/providers.shared";
...
export async function action({ request }: ActionFunctionArgs) {
  try {
    ...
    const isLocalProvider = LOCAL_INFERENCE_PROVIDERS.includes(parsedModel.providerId);
    ...
    } else if (!isProviderConfigured(parsedModel.providerId, validatedApiKeys)) {
      return new Response(
        JSON.stringify({
