...
export function useRoutingModelSettings() {
  const [settings, setSettings] = useState<RoutingModelSettings>(defaultRoutingModelSettings);
  const [definitions, setDefinitions] = useState<RoutingModelSettingDefinition[]>([]);
  ...
  const refresh = useCallback(async () => {
    try {
      setError(null);
      const data = await apiFetch<RoutingModelSettingsResponse>("/api/routing-model-settings");
      setSettings(data.settings);
      setDefinitions(data.definitions);
    } catch (err) {
      console.error("Failed to fetch routing model settings:", err);
      setError(err instanceof Error ? err.message : "Failed to fetch routing model settings");
    }
  }, []);
  ...
  const setEnabled = useCallback(async (key: RoutingModelSettingKey, value: boolean) => {
    const data = await apiFetch<{ settings: RoutingModelSettings }>("/api/routing-model-settings", {
      method: "PATCH",
