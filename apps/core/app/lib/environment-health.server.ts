...
export function getEnvironmentHealth(env: NodeJS.ProcessEnv = process.env): EnvironmentHealth {
  const missingKeys = ["DATABASE_URL", "BETTER_AUTH_SECRET", "EDUAI_API_KEY"].filter((key) =>
    isMissing(env[key]),
  );
  ...
  if (embeddingProvider === "local") {
    ...
    if (
      ["OPENROUTER_API_KEY", "GOOGLE_GENERATIVE_AI_API_KEY", "OPENAI_API_KEY"].every((key) =>
        isMissing(env[key]),
      )
    ) {
      missingKeys.push("OPENROUTER_API_KEY or GOOGLE_GENERATIVE_AI_API_KEY or OPENAI_API_KEY");
    }
