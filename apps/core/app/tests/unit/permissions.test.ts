...
describe("canDeleteMaterial", () => {
  ...
  ] as [CourseAccess, string, string, boolean][])(
    "access=%s own=%s → %s",
    (access, userId, uploadedBy, expected) => {
  ...
  it.each([
    ...
    ["ta", false],
    ["student", false],
  ] as [CourseAccess, boolean][])(
    "access=%s with null uploadedBy (orphaned material) → %s",
    (access, expected) => {
...
describe("canViewCourseChats", () => {
  ...
  it("instructor/unit defer to the resolved policy flag", () => {
    ...
    expect(canViewCourseChats("instructor", { "instructors.canViewCourseChats": false })).toBe(
      false,
    );
...
describe("resolvePolicyGate", () => {
  ...
  it("delegates manageEnrollments to manageEnrollmentsPolicyKey for every access level", () => {
    for (const access of ALL_ACCESS) {
      expect(resolvePolicyGate(access, "manageEnrollments")).toBe(
