...
function openCourseActions(
  button: HTMLElement = screen.getByRole("button", { name: /course actions/i }),
) {
...
describe("CoursesAdminView — mutation flows", () => {
  ...
  it("opens the edit dialog via the course actions menu, edits the name, and calls onEditCourse", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByDisplayValue(PUBLISHED_COURSE.name)).toBeInTheDocument(),
    );

    fireEvent.change(screen.getByDisplayValue(PUBLISHED_COURSE.name), {
...
describe("CoursesUnitAdminView — mutation flows", () => {
  ...
  it("greys out delete (data-disabled) when unitAdmins.canDeleteCourses is off (#807)", () => {
    ...
    openCourseActions();
    expect(screen.getByRole("menuitem", { name: /delete course/i })).toHaveAttribute(
      "data-disabled",
    );
  });
...
describe("CoursesMixedView", () => {
  ...
  it('shows only the "enrolled" section when the user has student courses only', () => {
    wrap(
      <CoursesMixedView courses={[STUDENT_COURSE]} taCourseIds={[]} enrolledCourseIds={["stu1"]} />,
    );
    expect(
      screen.getByRole("heading", { name: /courses you are enrolled in/i }),
    ).toBeInTheDocument();
    expect(screen.getByText("COSC 401")).toBeInTheDocument();
    expect(screen.getByText("Enrolled")).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: /courses you are assisting in/i }),
  ...
  it("hides draft (unpublished) courses from the enrolled section", () => {
    wrap(
      <CoursesMixedView
        courses={[
          STUDENT_COURSE,
          { ...STUDENT_COURSE, id: "stu2", code: "COSC 402", isPublished: false },
        ]}
        taCourseIds={[]}
