...
describe("useRoutingModelSettings", () => {
  ...
  it("setEnabled PATCHes the flag and adopts the server-returned settings", async () => {
    mockFetch
      .mockResolvedValueOnce(settingsResponse({ autoLlmEnabled: true, autoRulesEnabled: false }))
  ...
  it("setEnabled propagates the server error instead of swallowing it", async () => {
    ...
    await expect(result.current.setEnabled("autoRulesEnabled", true)).rejects.toThrow("forbidden");
  });

  it("refresh re-fetches the settings on demand", async () => {
    mockFetch
      .mockResolvedValueOnce(settingsResponse({ autoLlmEnabled: true, autoRulesEnabled: false }))
      .mockResolvedValueOnce(settingsResponse({ autoLlmEnabled: false, autoRulesEnabled: false }));
