...
// #1453 — same as the model list: a post-mutation refetch must skip the browser
// cache or the admin watches their own edit revert for the max-age window.
describe("useAiProviders cache busting (#1453)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(apiFetch).mockReset();
  });
  ...
  function lastListInit() {
    const calls = vi
      .mocked(apiFetch)
      .mock.calls.filter(([url]) => (url as string).startsWith("/api/ai-providers?"));
  ...
  it.each([
    ["createProvider", (r: any) => r.createProvider({ name: "new" })],
    ["updateProvider", (r: any) => r.updateProvider("prov-1", { name: "Renamed" })],
    ["deleteProvider", (r: any) => r.deleteProvider("prov-1")],
