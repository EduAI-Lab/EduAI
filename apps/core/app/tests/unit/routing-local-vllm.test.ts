...
describe("local-vllm routing", () => {
  ...
  it("maps tier-2 picks to tier 3 when VLLM_BASE_URL is set", () => {
    ...
    expect(normalizePickForLocalVllm({ kind: "exactTier", tier: 2, tieBreak: "carbon" })).toEqual({
      kind: "exactTier",
      tier: 3,
      tieBreak: "carbon",
    });
  ...
  it("leaves tier-1 picks unchanged", () => {
    process.env = { ...env, VLLM_BASE_URL: "http://cmps01:8001" };
    expect(normalizePickForLocalVllm({ kind: "exactTier", tier: 1, tieBreak: "energy" })).toEqual({
      kind: "exactTier",
      tier: 1,
      tieBreak: "energy",
    });
