...
describe("parseExtraExtensions", () => {
  ...
  it("parses a valid entry and sets all fields", () => {
    vi.stubEnv(
      ...
      '[{"id":"example","name":"Example","url":"http://localhost:9000"}]',
    );
  ...
  it("uses a custom color when provided", () => {
    vi.stubEnv(
      "VITE_EXTRA_EXTENSIONS",
      '[{"id":"x","name":"X","url":"http://x.com","color":"oklch(0.5 0.1 200)"}]',
    );
  ...
  it("passes through the description field", () => {
    vi.stubEnv(
      "VITE_EXTRA_EXTENSIONS",
      '[{"id":"x","name":"X","url":"http://x.com","description":"A demo"}]',
    );
  ...
  it("filters out entries missing a required field", () => {
    vi.stubEnv(
      "VITE_EXTRA_EXTENSIONS",
      '[{"id":"x","name":"X"},{"id":"y","name":"Y","url":"http://y.com"}]',
    );
    const apps = parseExtraExtensions();
    expect(apps).toHaveLength(1);
  ...
  it("parses multiple valid entries", () => {
    vi.stubEnv(
      "VITE_EXTRA_EXTENSIONS",
      '[{"id":"a","name":"A","url":"http://a.com"},{"id":"b","name":"B","url":"http://b.com"}]',
    );
