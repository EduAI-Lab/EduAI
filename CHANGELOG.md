...
- [core] auth: Gate `GET /api/ai-providers` and `GET /api/ai-models` as ADMIN-only (#292, §13) — both were publicly readable; they now return 401 anonymous / 403 non-admin, matching the existing POST/PATCH/DELETE gates; `GET /api/ollama-models` regression-checked (already ADMIN-only); only the admin UI consumed these endpoints, so no non-admin surface breaks. (#303, @abdullahmoh21, 2026-06-05)
- [core] api: Close the §4 user-management gaps (#292) — new `GET`/`PATCH /api/me` self-profile route for any authenticated user with a dedicated `{name, image}` whitelist schema (role/isActive unreachable; unknown keys stripped); self-role-change guard in `PATCH /api/users/:id` (403 `Cannot change your own role`, mirroring the self-deactivation lockout — also fixes a latent double `request.json()` read on self-PATCHes); `authorizedUnits` assignment via `PATCH /api/users/:id` (ADMIN-only, `DepartmentSchema`-validated, `422 ROLE_MISMATCH` for non-UNIT_ADMIN targets, same-request promotion supported); add the missing `UNIT_ADMIN` value to the create/update user schema enums (UNIT_ADMIN creation was silently impossible). Unit guard coverage + a per-role GET/PATCH `/api/me` integration round-trip (STUDENT/INSTRUCTOR/UNIT_ADMIN/ADMIN). (#297, @abdullahmoh21, 2026-06-05)
- [core] api: Gate AI chat per the §10 matrix (#292, partial) — `POST /api/chat` with a course context now resolves course access for the acting user (proxyUser included): 404 for missing courses, 403 with no relationship, students require an active enrollment AND a published course (inactive enrollment blocks new chats but never own-history reads); chats without a course context are ungated; new `DELETE /api/chats/:chatId` → 204 (owner-only, ADMIN may delete any; non-owners get the same 404 as missing chats — no existence leak). The §10 chat-metrics endpoint is deferred: no chat↔course persistence exists (`Chat` lacks `courseId`; `AIInteraction` is never written) — needs a team modeling decision before it can return real data. (#302, @abdullahmoh21, 2026-06-05)
- [core] api: Layer user-OAuth RBAC + answer-key stripping onto the questions endpoints (#292, §9) — `GET /api/questions` and `GET /api/questions/:id` user paths now resolve course-scoped access (ADMIN/UNIT_ADMIN(D)/INSTRUCTOR(C)/TA(C); STUDENT → 403 — students never read questions directly; a platform INSTRUCTOR not enrolled in the course is also 403); `POST /api/questions` adds the same course scoping (TA-and-up via enrollment, replacing the platform-role allowlist); `PATCH /api/questions/:id` user path requires rank ≥ 2 with the TA own-only carve-out (`Question.createdBy === user.id` + TA enrollment); `stripAnswerForStudents` (#293) wired through the list and single-question response paths per §19; all service-key paths unchanged. Route coverage updated for enrolled/non-enrolled × role, TA own-vs-other edits, and answer preservation for TA-and-up. (#301, @abdullahmoh21, 2026-06-05)
- [core] api: Enforce the §7 course-materials RBAC matrix (#292) — `POST /api/courses/:id/materials` now blocks STUDENT uploads (TA-and-up only; previously any enrolled user could upload); `GET` adds the student publish gate (enrolled student in an unpublished course → 403; TA/INSTRUCTOR exempt) and a clean 404/403 split via `resolveCourseAccess`; new `DELETE /api/courses/:id/materials/:materialId` → 204 (ADMIN/UNIT_ADMIN(D)/INSTRUCTOR(C), plus TA own-only via `uploadedBy` (#294), null-owner rows denied to TAs; hard delete — `CourseMaterial` has no `deletedAt`, chunks cascade). Unit matrix coverage + an INSTRUCTOR upload → list → delete integration cycle and TA own-vs-instructor delete cases. (#300, @abdullahmoh21, 2026-06-05)
- [core] api: Add enrollment management with the §6 instructor-floor invariant (#292) — new `POST /api/courses/:id/enrollments` (ADMIN/UNIT_ADMIN(D) any role; INSTRUCTOR(C) may add STUDENT/TA but never a fellow INSTRUCTOR; `409 ALREADY_ENROLLED` on the unique constraint, `422 USER_NOT_FOUND`); new `PATCH`/`DELETE /api/courses/:id/enrollments/:enrollmentId` (role changes and soft removal via `isActive=false`; promotions to INSTRUCTOR and any change/removal of an INSTRUCTOR enrollment are ADMIN/UNIT_ADMIN-only); **instructor-floor invariant** enforced transactionally — any demotion/deactivation that would leave a course with zero active instructors is rejected with `409 { error: "INSTRUCTOR_FLOOR_VIOLATION", currentInstructorCount }`, with no ADMIN override; `GET /api/courses/:id/enrollments` user-OAuth path now applies §6 (TA-and-up see all; enrolled STUDENT → 403; service-key path unchanged). Unit matrices for all three mutators + route gates and an integration lifecycle (create-with-instructor → add second → remove first → removing last 409s). (#305, @abdullahmoh21, 2026-06-05)
- [core] api: Enforce the §8 course-topics RBAC matrix (#292) — user-OAuth `GET /api/courses/:id/topics` now requires a course relationship (ADMIN/UNIT_ADMIN(D)/INSTRUCTOR(C)/TA(C), STUDENT(C + published), 404 for missing course); `POST` opens from ADMIN-only to rank ≥ 2 (instructors can finally create topics in Core); new `PATCH /api/courses/:id/topics/:topicId` rename route (200/404/409) via `updateCourseTopic`; `DELETE` opens from ADMIN-only to rank ≥ 2; TA own-only edit/delete via `CourseTopic.createdBy` (#294) with null-owner rows denied to TAs; service-key paths unchanged on all verbs. Unit matrix coverage (student/TA/instructor × verb, TA own-vs-other-vs-ownerless) plus an INSTRUCTOR create → PATCH → soft-delete integration lifecycle and the student publish-gate round-trip. (#299, @abdullahmoh21, 2026-06-05)
- [core] api: Enforce the §5 course-management RBAC matrix (#292) — `GET /api/courses` now requires a session and scopes results per caller (ADMIN all, UNIT_ADMIN authorized units, INSTRUCTOR/TA enrolled regardless of publish state, STUDENT enrolled + published, with per-enrollment-role publish gating for mixed-role users); `GET /api/courses/:id` gated by `resolveCourseAccess` (403 for unrelated users, student publish gate; service-key path unchanged); `POST /api/courses` opens to UNIT_ADMIN with `403 DEPARTMENT_NOT_AUTHORIZED` outside `authorizedUnits` (null department never matches); `PATCH /api/courses/:id` moves to rank-based gating (ADMIN/UNIT_ADMIN(D)/INSTRUCTOR(C)) and blocks UNIT_ADMIN from moving a course outside their units; new `DELETE /api/courses/:id` soft-delete (`deletedAt`, 204); `Course.department` writes validated against `DepartmentSchema` (§19). Unit + integration coverage including the grad-TA mixed-enrollment list case and the soft-delete disappearance cycle. (#298, @abdullahmoh21, 2026-06-05)
- [core] db: Add owner FK columns for TA own-only RBAC (`rbac_owner_columns` migration) — `CourseMaterial.uploadedBy` and `CourseTopic.createdBy`, both nullable `String?` FKs to `User` with `onDelete: SetNull` and no backfill (null owner = "TA delete not permitted, INSTRUCTOR-and-up only"); wire writes so `POST /api/courses/:id/topics` and `POST /api/courses/:id/materials` persist the session user, and the service-key path persists null; unit + integration tests for owner threading on both auth paths. (#294, @abdullahmoh21, 2026-06-05)
- [core] auth: Add the RBAC keystone helpers (#292 foundation) — `resolveCourseAccess(user, courseId)` implementing the shared §3 contract (`AccessLevel` with `level`/`rank`, ADMIN → UNIT_ADMIN unit-lock → active enrollment resolution, null department never matches UNIT_ADMIN), Core-local `resolveCourseAccessWithCourse` returning the fetched course row to distinguish 404 from 403, `buildCourseListFilter` with per-enrollment-role publish gating (grad-TA case), `stripAnswerForStudents` §19 answer-visibility serializer, and `DepartmentSchema` canonical subject-code validator (`app/lib/departments.ts`); document the shared contract in `rbac-matrix.md` §3 for AI Tutor (#295) and QM (#296). (#293, @abdullahmoh21, 2026-06-05)
- [core] tests: Add 41 unit tests for the RBAC keystone (`course-access.server.test.ts`, `departments.test.ts`) — every resolution branch (each role, null-department UNIT_ADMIN, lazy `authorizedUnits` fetch, inactive enrollment, no relationship), list-filter publish gating per enrollment role, answer-strip behaviour per access level, and subject-code casing rejection; add shared RBAC test fixtures (`app/tests/helpers/rbac.ts`: `seedUser`/`seedCourse`/`enroll`/`mockSession`/`cleanupRbac`) for the #292 family's role × route matrices. (#293, @abdullahmoh21, 2026-06-05)
