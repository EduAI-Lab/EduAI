...
- [core] feat: ADHD Assist Phase 3 oversight (#493) — `auditAndMaybeRewrite` audits Assist ON drafts; deterministic anchor fix preserves trailing redirect questions; LLM rewrite with draft fallback on provider failure; separate `oversightDurationMs` and oversight token fields in telemetry; overseen streaming replies persisted. Env: `ADHD_ASSIST_OVERSIGHT` (default on). Requires #532 telemetry. (#493, @Ayyhab, 2026-06-09)
