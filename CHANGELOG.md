...
=======
- [monorepo] auth: Unify the auth frontend across apps so all three apps share a consistent sign-in surface. (#PR, @evanbones, 2026-06-01)
- [core] auth: Enforce ADMIN-only access on `GET`/`POST /api/courses`; migrate remaining `professor` → `instructor` role references. (#PR, @abdullahmoh21, 2026-06-01)
- [monorepo] auth: Rename the `PROFESSOR` role to `INSTRUCTOR` across all three apps — admin UI, auth schemas, middleware, and tests. (#PR, @abdullahmoh21, 2026-06-01)
- [question-maker] refactor: Switch `eduaiService` to a pino logger and silence logs in tests. (#PR, @abdullahmoh21, 2026-06-01)
- [question-maker] tests: Migrate backend integration tests from JWT auth to Core session-cookie auth; remove the obsolete `auth.integration.test.js` (register/login removed with JWT). (#PR, @abdullahmoh21, 2026-06-01)
- [ai-tutor] chore: Upgrade `streamdown` to 2.5.0. (#PR, @abdullahmoh21, 2026-06-01)

### Fixed
- [question-maker] api: Fix variant push to Core — support CUID string primary topic ids, lowercase `difficulty` / `reasoningLevel` enum values, return Core topic ids in the `INVALID_TOPIC_IDS` response, count name-updated topics in the sync-topics synced total, query JSON `question_order` with the `->>` operator, and wrap the cursor insert in a savepoint to survive a unique-key race. (#PR, @abdullahmoh21, 2026-06-01)
>>>>>>> 14234c2 (chore: doc updates)
---
