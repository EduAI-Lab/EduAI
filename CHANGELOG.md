...
- [monorepo] docs: Add sprint step **L13** — admin-side `webToolsEnabled` Feature Toggle that gates `webSearch` + `fetchPage` (default OFF for the ADHD latency research phase) so `supportsTools: true` means `getInformation` is always there plus web tools only when an admin opts in; web-tool source and Firecrawl wiring remain in the codebase ([#348](https://github.com/EduAI-Lab/EduAI/issues/348)).
...
### Changed
- [monorepo] docs: Define the `supportsTools` invariant at the top of [`TEAM_CHAT_LATENCY_SPRINT_GUIDE.md`](docs/rag-ai/latency/eduai-summer-2026/TEAM_CHAT_LATENCY_SPRINT_GUIDE.md) and in [`TEAM_CHAT_LATENCY_AND_TOOLS.md`](docs/rag-ai/latency/eduai-summer-2026/TEAM_CHAT_LATENCY_AND_TOOLS.md) — `false` → small model, no tools, fast path for simple questions; `true` → bigger model, `getInformation` always registered, escalated when L04 intent fires on detailed / course questions.
- [monorepo] docs: Rescope sprint step **L13** in [`TEAM_CHAT_LATENCY_SPRINT_GUIDE.md`](docs/rag-ai/latency/eduai-summer-2026/TEAM_CHAT_LATENCY_SPRINT_GUIDE.md) and [`TEAM_CHAT_LATENCY_AND_TOOLS.md`](docs/rag-ai/latency/eduai-summer-2026/TEAM_CHAT_LATENCY_AND_TOOLS.md) per 2026-05-26 PI meeting — `webSearch` / `fetchPage` and Firecrawl wiring are **no longer being deleted**; instead they are gated behind an admin-side `webToolsEnabled` Feature Toggle (default OFF) so the ADHD latency study can measure RAG-only behaviour while the code stays available to flip on later. Spec, invariant, mermaid diagram, assignment matrix, end-of-week checklist, and code-map row all updated to match ([#348](https://github.com/EduAI-Lab/EduAI/issues/348)).
