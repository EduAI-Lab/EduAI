...
### Added
- [core] rag: Cache `getCourseRagSettings` in-memory (60-min TTL, tunable via `COURSE_RAG_SETTINGS_CACHE_TTL_MS`) — avoids a DB round-trip on every `findRelevantContent` call; null results (unknown courseId) are also cached; `invalidateCourseRagSettingsCache()` is called on `PATCH /api/courses/:id/rag-settings` so updates apply immediately without waiting for TTL expiry. (#366, @ammaarm128, 2026-06-05)
- [core] tests: 7 unit tests (`courses.rag-settings-cache.test.ts`) for the RAG settings cache — covers cache hit (single DB call for repeated reads), TTL expiry triggers re-fetch, invalidation forces re-fetch, invalidating one course does not evict others, null result is cached, and independent entries per courseId. (#366, @ammaarm128, 2026-06-05)
