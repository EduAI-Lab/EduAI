...
- [core] feat: ADHD Assist Phase 3 oversight (#493) — `auditAndMaybeRewrite` in `adhd-oversight.ts` audits Assist ON drafts against structural metrics; deterministic anchor fix for common drift (`**Top summary**`, `**Next?**`), LLM rewrite fallback; wired into `POST /api/chat` when `ADHD_ASSIST_OVERSIGHT` is enabled (default on). Buffers streaming responses so users receive the overseen text. Telemetry logs `oversightRewritten`, `oversightMethod`, `preStructuralPass`. (#493, @Ayyhab, 2026-06-09)
- [core] tests: Unit tests for Phase 3 oversight (`adhd-oversight.test.ts`) — env flag, S1-on deterministic fix, pass-through, LLM fallback. (#493, @Ayyhab, 2026-06-09)
