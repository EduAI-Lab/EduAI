...
- [core] feat: OpenRouter chat provider — register `openrouter` in the AI SDK provider registry so users can chat with cloud models via a browser-stored OpenRouter API key and model ids like `openrouter:google/gemini-2.5-flash`; seed default OpenRouter models; Settings and chat API key UI updated. (#PR)
- [core] tests: Unit tests for OpenRouter provider registry wiring, model identifier parsing, and API key settings UI. (#PR)
